...
- **Separar `nombre_pila` y `apellido` en `simon_brokers` y nombre del cliente en `broker_shortlists`** — hoy `nombre` guarda todo junto ("Abel Antonio Flores Nava") y el preview WA usa `firstName()` (split por espacios, primer token) para mostrar "Selección de Abel para Luis" — funciona 95% bien pero se rompe en casos como "María José Pérez" → "María" (perdés "José"). Migración separaría ambos campos en BD + agregaría inputs en `/admin/simon-brokers`. Bajo riesgo, alta opcionalidad: el código actual `firstName()` sigue funcionando para registros viejos sin tocar nada. Reactivar si aparece broker con nombre compuesto y se ve raro el preview, o cuando se quiera precisión 100%. Commit que dejó el split provisorio: `ce3faba` (7 may 2026).
