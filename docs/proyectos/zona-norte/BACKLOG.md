...
**🎯 Próxima sesión:** **Fase 2 — Pipeline Casas/Vivienda ZN** (mayor valor): la carga ya se hizo (305 casas ZN activas con contacto, FK `id_condominio_master` + matcher nombre+GPS ya aplicados). **Lo pendiente ahora es la vista `v_mercado_casas` + feed `/ventas/casas` + cron.** La tabla `condominios_master` (45 condominios, mig 260+261) ya existe aislada. Después: extender pipeline a Urubó (~179 terrenos, `land_m2` ya corregido). Ver `project_sonda_suelo_zn_urubo_jun2026` + `DISENO_PIPELINE_CASAS_VIVIENDA.md`. **Recordar reactivar el workflow `auditoria_diaria_sici_v3.0` en n8n** si sigue desactivado.
