...
- 🟡 **#7.1 — Fase 3 alquiler ZN: discovery dedicado (EN CURSO — C21 ✅ implementado y validado 31-may; falta Remax + BI).** Replicar lo que venta hizo en su Fase 3, para los 3 portales:
  ...
  - ⬜ Crear `flujo_discovery_remax_alquiler_zonanorte` (endpoint todo-SC + "Filtrar Solo Alquileres" + polígono, NO slug) y `flujo_discovery_bien_inmuebles_alquiler_zonanorte` (filtro `nomb_barri` → point-in-polygon GPS), análogos a los `_zonanorte` de venta + su blindaje EQ par.
