...
- 🟡 **#7.1 — Fase 3 alquiler ZN: discovery dedicado (EN CURSO — C21 + Remax ✅ validados 31-may; falta BI).** Replicar lo que venta hizo en su Fase 3, para los 3 portales:
  ...
  - ⬜ **BI (Paso 5):** `flujo_discovery_bien_inmuebles_alquiler_zonanorte` — cambio clave: filtro client-side `nomb_barri='equipetrol'` → point-in-polygon GPS (POST único sin grid ni paginación) + blindaje BI EQ. Último portal (2 props ZN, cobertura completa).
