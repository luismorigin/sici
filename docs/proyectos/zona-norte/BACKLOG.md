...
- ⬜ **#7.1 — Fase 3 alquiler ZN: discovery dedicado (LO QUE FALTA PARA PRODUCCIÓN).** Replicar lo que venta hizo en su Fase 3, para los 3 portales:
  ...
  - **📋 Plan de implementación:** `docs/proyectos/zona-norte/PLAN_FASE3_DISCOVERY_ALQUILER_ZN.md` §0 + §0.1. **MVP (corregido tras catch de método del director):** la priorización de portales NO se decide con el volumen de Equipetrol (el mix cambia por zona — en ZN es Remax 30 vs C21 1, opuesto a EQ; y ese dato está sesgado porque solo Remax llega a ZN). **Primero un spike de inventario alquiler ZN por portal** (consultar C21/Remax/BI por el polígono ZN, como el PoC de venta del 20-may) → eso decide alcance. **Por defecto clonar los 3** (Remax patch + C21 grid + BI), sin descartar ninguno sin datos. + blindar marcar-ausentes (`zona IN (6 EQ) OR zona IS NULL`) en los portales cuyo scrape no traiga ZN. LIMITs condicionales. Lo zone-agnostic (enrichment/merge/verificador) y el riesgo-EQ-nulo del filtro siguen firmes.
