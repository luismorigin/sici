...
| 250 | zona_norte_poligono_y_backfill | **Polígono macro Zona Norte + ampliar CHECK de zonas + backfill legacy.** Base del proyecto ZN (expansión geográfica más allá de Equipetrol). |
| 251 | blindajes_matching_y_snapshot | **Blindajes para que Zona Norte no contamine métricas de Equipetrol.** Matching same-zone (`generar_matches_por_nombre`) + blindaje snapshot global EQ. |
| 252 | blindaje_matching_trigram_zona | **Blindaje extendido:** `generar_matches_trigram` con same-zone matching (no cruza macrozonas). |
| 253 | hitl_zona_norte_separado | **Separar el HITL de Zona Norte del de Equipetrol** vía estado distinto (`pendiente_zona_norte`). Trigger `trg_separar_hitl_por_macrozona` (usa `zona_general`, escala a Urubó). |
| 254 | microzonas_zona_norte | **Cargar 14 microzonas de Zona Norte** (grilla anillos×avenidas, recortadas con `ST_Difference`) + refactor trigger HITL. 73 pm + props redistribuidos. Rollback en `_rollback.sql`. |
| 255 | snapshot_absorcion_v4_dinamico | ⚠️ **NO APLICAR — DESCARTADA** (29-may-2026). Blueprint del snapshot multi-macrozona dinámico (`FOR macrozona IN SELECT DISTINCT zona_general...`). Queda como **referencia de diseño** del patrón por `zona_general`. |
| 256 | snapshot_alquiler_zonas | **Snapshot: alquiler blindado a EQ en la serie global + serie de alquiler por-zona** (10 celdas ZN con ROI). Ticket #7 Zona Norte (FIX A). Venta intacta. |
| 257 | zona_general_en_vistas | **Columna `zona_general` (macrozona) en `v_mercado_venta`/`v_mercado_alquiler` + helper `macrozona_de(text)`.** Cimiento (Fase 0) del plan de parametrización por macrozona — aísla Equipetrol de Zona Norte. `CREATE OR REPLACE VIEW` de ambas vistas agregando `zona_general` vía `LEFT JOIN (SELECT nombre, MAX(zona_general) FROM zonas_geograficas WHERE activo GROUP BY nombre)` (el `GROUP BY nombre` colapsa el doble polígono 'Equipetrol Norte' → 1:1 por construcción). Columna aditiva (no rompe consumidores). `macrozona_de(zona)` para consumidores que van a `propiedades_v2` directo. Verificado: venta EQ=384/ZN=399, alq EQ=143/ZN=105, total sin cambios (no duplica), 0 sin macrozona, helper==columna. Plan: `docs/proyectos/zona-norte/PLAN_PARAMETRIZACION_MACROZONAS.md`. |
| 258 | ssot_macrozona_resumen_buscar | **`resumen_mercado()` y `buscar_propiedades()` → default Equipetrol vía `zona_general='Equipetrol'`** (elimina la allowlist hardcodeada de 5 zonas, **corrige el bug del 'Eq. 3er Anillo' faltante**). Decisión 1-jun-2026: el 3er Anillo ES Equipetrol → 6 zonas. Verificado: `resumen_mercado('venta')` default = 384 (6 zonas EQ, 0 ZN). Fase F3 de `PLAN_PARAMETRIZACION_MACROZONAS.md`. |
