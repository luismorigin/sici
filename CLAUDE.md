...
15. **Skills de auditoría feed/cola — ⚠️ ALCANCE PARTIDO desde el 21-jul** (changelog en cada `.command.md`; fuente de verdad = `scripts/auditoria-*/` del repo, las skills viven gitignored en `.claude/commands/`):
    ...
    - **Shadow (híbrido)** — fuente de verdad en `scripts/deptos-equipetrol/` (NO `scripts/auditoria-*/`): `/audit-cola-shadow` (matching + dedup apart-hotel **y republicación SIN código repetido**, **12 superficies** — 1 sin-match+nombre · 2 auto-match riesgoso · 3 dedup · 4 el lector dudó · **4b el lector dudó y NO hay match** (22-ago, backlog acumulado) · 5 match LEJOS del edificio (>800 m) · 6 **el edificio se contradice sobre su estado de obra** (6-ago) · 7 **mismo depto a precios incompatibles** (8-ago, >30% entre avisos con igual pm+área+captador; el dedup NO puede verlos porque el precio es parte de su clave de grupo) · **9 el aviso publica SOLO en Bs y no está tagueado `bob`** (20-ago; la 8 se midió y se DESCARTÓ, queda documentada en la skill) · **10 la prop y su edificio en macrozonas distintas** (20-ago) · **11 dos fichas del CATÁLOGO que el matcher ve como una** (28-ago, mig 345 `v_colisiones_catalogo` — la única que no mira propiedades) · **11b una prop cayó del lado equivocado de una colisión** (31-ago; conjunción nombre+distancia: cada mitad sola daba 7 y 5 falsos positivos, juntas 1 y era real) —, respeta `campos_bloqueados`) + `/audit-deptos-shadow` (drift = re-lectura del anuncio vs lo guardado). $0 read-only sobre `propiedades_v2`; veredicto de matching = subagentes-lectores. 🔁 **El audit ya NO es la única puerta del dedup**: desde el PR #64 la republicación por **slug reescrito** se resuelve en la CAPTURA — **C21 y Remax** desde el 28-ago (ver Quick Context).
...
│   │                        🔴 ÚNICA EXCEPCIÓN al filtro de zona: el índice de códigos del
│   │                        "slug reescrito" se cruza contra shadow COMPLETO (el código es
│   │                        único en su portal → detecta aunque el aviso cambie de zona;
│   │                        avisa `⚠️ cambió de zona, revisar`). La fuente se deduce del
│   │                        HOST, no del campo `fuente`: C21 pone el código al principio
│   │                        (`/propiedad/<cod>_`) y Remax al final, en DOS partes
│   │                        (`<listado>-<unidad>`) — y las dos importan, porque el sufijo
│   │                        distingue UNIDADES (Berchatti -15/-16/-17 son tres deptos)
