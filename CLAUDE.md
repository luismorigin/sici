...
| **Proyecto Zona Norte** | `docs/proyectos/zona-norte/` — **prototipo de la arquitectura multi-macrozona Simón Santa Cruz** (ADR-009). Fases 1-4 en producción 27-may-2026: polígono + blindajes (mig 250-253), workflows discovery zona-agnósticos activos (cron 1:15/1:45 AM), HITL separado, dark launch funcional con 388 props ZN venta + 53 alquiler procesadas. Workflows ZN son universales multi-macrozona (lectura zona-agnóstica de BD — para agregar Urubó/Polanco editar 1 array). **Strangler pattern: Equipetrol producción NO se toca.** Empezar por README → DECISIONES.md (9 ADRs) → BACKLOG.md (tickets por prioridad) → operacion.md (kill-switch). Próximo: ticket #1.5 (cargar pm con cluster GPS) + ticket #6 (construir `/mercado/zona-norte`) |
