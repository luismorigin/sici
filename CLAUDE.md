...
| **Simon Broker (MVP venta + Fase 2 alquileres mergeados)** | `docs/broker/` — PRD, backlog, decisiones. MVP venta en `main` (merge S3 `05bc1eb`, 23 Abr 2026) + Fase 2 alquileres (merge `65ccc4b`, 24 Abr 2026). **Venta:** `/broker/[slug]` modo broker con estrella verde + chips + ACM inline, ruta pública `/b/[hash]` reusando feed `/ventas` con `publicShare` (inmobiliaria visible + WA con corazones marcados), badges precio (agente cambió vs TC se movió), mensajes WA broker→agente y cliente→broker. Tabla `simon_brokers` (migración 231) reemplaza `lib/brokers-demo.ts` — admin UI `/admin/simon-brokers`. Shortlists agrupadas por cliente + alerta "cliente existente" (Nivel 1). **Fase 2 alquileres:** `/broker/[slug]/alquileres` (modo broker sobre feed alquileres), tabs venta/alquiler en banner broker, `/b/[hash]` ramifica por `tipo_operacion` del primer item (venta→VentasPage, alquiler→AlquileresPage con header público + FAB mapa mobile), snapshot BOB en `broker_shortlist_items.precio_mensual_bob_snapshot` (migración 233), feedback del cliente al broker via `broker_shortlist_hearts` (migración 234) visible en editor "N de M marcadas", validación teléfono E.164 en ShortlistSendModal. Migraciones 228/229/230/231/233/234. Fase 2 planificación original en `docs/broker/BACKLOG.md` sección "Fase 2 — HECHA". |
