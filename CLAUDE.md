...
**Deptos Equipetrol → HÍBRIDO** (✅ en main desde 18-jul-2026, PR #22. 🚀 **Desde el 21-jul TODA la app pública de Equipetrol LEE SHADOW** — ya NO es dark-launch; ver §Sistema de precios. 🏁 **El cutover YA OCURRIÓ EN LOS HECHOS (10-ago-2026)**: el founder dio de baja Firecrawl y el servidor n8n a fines de julio → **`propiedades_v2` congelada desde el 28-jul, `market_absorption_snapshots` cortada el 27-jul, `workflow_executions` mudo. n8n NO vuelve.** Shadow es la base de facto — y **prod NO afecta a shadow**: son tablas separadas sin dependencia. ✅ **TIEMPO 1 EJECUTADO (11-ago): `propiedades_v2` → `propiedades_v2_archivo`.** Los 4 evals pasan, **cero hallazgos fuera de la predicción**: feeds 354/182 idénticos, bot idéntico, las 6 páginas con el mismo peso, las 4 capturas corriendo. Se rompió lo predicho y solo eso — **el admin, `buscar_unidades_reales` y `buscar_extras`** (esas 2 las usan el CMA del broker y la creación de shortlists). ✅ **TIEMPO 2 EJECUTADO (17-ago-2026): la tabla viva se llama `propiedades_v2`.** Las 5 condiciones de entrada se cerraron o se descartaron ese mismo día (la 1 estaba redactada de más; la 3 y la 4 ya se cumplían y nadie lo había medido). Se hizo con **ensayo en vacío previo** (el movimiento completo dentro de una transacción con `ROLLBACK`), predicción firmada y **cero números movidos**. Queda vivo el **atajo** `propiedades_v2_shadow` (una VISTA) hasta terminar la limpieza — 6 funciones, 53 puntos de código y 6 skills. Ver regla #3 y `docs/RETOMAR.md`. 🔴 **Fuente de verdad única: `scripts/deptos-equipetrol/INVENTARIO_CUTOVER_2026-08-10.md`** — `CUTOVER_DATA_PLAN.md` quedó SUPERADO (lleva banner; su escenario ya no existe)): reemplazar el n8n de deptos-venta por el flujo híbrido (discovery propio + reader-extendido en sesión = MOAT + apply a entorno SHADOW aislado `propiedades_v2_shadow`, migs 268/269 + helper 271 + normalización TC-nuevo 272 ⚠️ [las migs 268 y 276 colisionan con main/frontend → renumerar al cutover, ver `docs/migrations/MIGRATION_INDEX.md`]). Reader spec `scripts/deptos-equipetrol/READER_SPEC.md` **v4.2** (precio/TC/dorms/baños/piso/amenidades+extra/equipamiento-3-baldes/parqueo/baulera/estado/amoblado/equipado/multiproyecto+bloque-piso-completo). **TC nuevo** (unificación oficial=paralelo) congelado en `TC_NUEVO_DECISION.md` (default vs `oficial_viejo`; **principio de arquitectura: normalización = frontera de acceso, crudo+tag adentro / normalizado afuera** — portable, para la Plataforma Híbrida Genérica). Ciclo end-to-end en skills: captura Equipetrol `/cron-deptos-ventas` + `/cron-deptos-alquiler` · captura **Zona Norte** `/cron-deptos-ventas-zn` + `/cron-deptos-alquiler-zn` (31-jul: mismos scripts, **todos los pasos pasan `--zona=zona-norte`**; el default de `lib/zonas-hibrido.mjs` es `equipetrol` y un paso sin el flag **no falla, hace la cosa equivocada en silencio**), 🔁 **El discovery caza el "slug reescrito" de C21** (PR #64, 4-ago-2026): C21 arma la URL como `/propiedad/<codigo>_<slug>` y **reescribe el slug cuando el captador edita el aviso** (típicamente al BAJAR el precio) → la URL cambia y el mismo depto entraba dos veces, con dos precios, contaminando la mediana de su microzona. Se detecta por el código —único por AVISO, no por URL— y **NO se filtra: se captura** (el precio nuevo es el vigente; saltearla dejaría el viejo para siempre) y el cargador marca la vieja `duplicado_de` en el `--apply`. Es el ÚNICO punto donde el apply toca filas que no vienen en el material. Detalle: memoria `project_c21_slug_reescrito`. Auditoría `/audit-cola-shadow` (matching+dedup, **las 2 zonas**) + `/audit-deptos-shadow` (drift). **Parte matutino: `/revisar-routines`** — al pedir "revisar las routines / cómo salió lo de anoche", 🔴 **leer PRIMERO los LOGS de las 5 routines** (`output/` → `cron-deptos-ventas-log.md` + `cron-deptos-alquiler-log.md` + **`cron-deptos-ventas-zn-log.md`** + **`cron-deptos-alquiler-zn-log.md`** + `audit-cola-shadow-log.md`), **NO la BD**: la routine `audit-cola-shadow` es read-only y deja su veredicto en un log, NO en `matching_sugerencias` (esa tabla es n8n/PROD, régimen viejo que el feed ya no lee → confundirlas fue el bug del 23-jul). Son las 5 routines, no solo el audit (el gate de captura vive en el log de ventas), y **cada zona tiene su log propio**: leer solo los 3 de Equipetrol deja Zona Norte invisible. Si una routine de ZN todavía no está agendada, **decirlo** en vez de omitirla. 🔴 Y el `lastRunAt` de la scheduled-task NO alcanza para saber el ORDEN: el 30-jul ventas marcaba 01:17 y su log decía que arrancó 06:27, con el audit corriendo ANTES de la captura — **comparar los horarios que declaran los logs entre sí**. BD solo para confirmar cifras, contra tablas `*_shadow`. Cutover: **`scripts/deptos-equipetrol/INVENTARIO_CUTOVER_2026-08-10.md`** (fuente única; el viejo `CUTOVER_DATA_PLAN.md` quedó como historia) · auditorías post-cutover: `AUDITORIAS_POST_CUTOVER.md`. Estado: memoria `project_checkpoint_deptos_hibrido`.
...
> 🔴 **LAS 3 RPC DEL BOT SON `resumen_mercado`, `buscar_propiedades` y `buscar_similares`** — verificado en el código real del workflow (`lab-kapso/workflows/simon/workflow.js`), no en la documentación. **NO son las del sitio** (`buscar_unidades_simple_shadow` y compañía). Confundirlas costó **19 días de bot caído** (24-jul→12-ago): la mig 317 se justificó diciendo *"las 3 RPC del bot son SECURITY DEFINER"* —cierto de las tres que nombra— sin notar que el bot usa otras. **No se afirmó sin verificar: se verificó el objeto equivocado.** Migs 320 (DEFINER + search_path), 321 y 325 (timeout). 🆕 **Migs 330/331 (19-ago): las 3 devuelven el campo `amenidades`** — array de vocabulario CERRADO (15 valores), ordenado y sin duplicados, que combina las amenidades de la propiedad con las del edificio normalizadas (`amenidades_normalizadas(id)`). Cobertura: **355/391 en venta, 173/186 en alquiler**. 🔑 **`[]` significa "no tenemos el dato", NO "no tiene"** — 36 y 13 respectivamente. Memoria `project_bot_caido_tres_semanas`. Los precios públicos son del régimen nuevo (~31% menores en venta; alquiler Bs igual). **Las funciones/vistas de PROD siguen en régimen viejo** (ZN ya pasó al híbrido el 30-jul → régimen nuevo; n8n está apagado desde el 28-jul). 🔴 **Esas funciones viejas son inofensivas SOLO porque leen una tabla congelada** — calculan con `precio_normalizado()`: si `propiedades_v2` pasara a ser la base buena, inflarían ~47% sin dar error. ✅ **Corregido el 14-ago:** de los consumidores que este párrafo listaba, la **creación de shortlists NUNCA usó `buscar_unidades_reales`** (lee `v_mercado_venta_shadow` desde el 21-jul) y `buscar_extras` ya tenía gemela `_shadow`. El **CMA v1 del broker se apagó** (410; lo reemplaza el ACM del PR #71) — llevaba 3 días roto sin que nadie lo notara y `broker_cma_uso` tiene 0 filas históricas. Y el **funnel premium se apagó el mismo día** (páginas borradas, rutas → `/ventas`). 👉 **`buscar_unidades_reales` NO tiene llamadores vivos (19-ago-2026).** Esta línea decía "queda con UN solo llamador: `/admin/propiedades`" y **era falso**: esa página no llama ninguna RPC. Su único llamador real vivía donde ningún `grep` del repo lo veía — **`generate_advisor_snapshot()`, en pg_cron**, y además `simon-advisor` (OTRO repo, deploy propio) la usa como fallback en vivo. El cron se **desagendó** el 19-ago al pausar el Advisor. 🔴 **Por eso NO se desarmó en la mig 328**: renombrarla rompería una app publicada. Sigue calculando con la fórmula vieja sobre la base viva, junto con `razon_fiduciaria_texto` y `generar_razon_fiduciaria` — deuda declarada en `simon-advisor/RETOMAR_ADVISOR.md`. Es el motivo del plan en dos tiempos: `scripts/deptos-equipetrol/INVENTARIO_CUTOVER_2026-08-10.md` §6-7. Detalle: `scripts/deptos-equipetrol/LANZAMIENTO_TC_NUEVO.md`.
...
| **TC sospechoso criterios** | `docs/canonical/TC_SOSPECHOSO_CRITERIOS.md` — badge "Confirmar tipo de cambio". 🔴 **El badge tiene DOS vías y una está MUDA (medido 19-ago):** la **A** (mig 311, el aviso menciona TC 7/6.96) marca 100 avisos; la **B** (mig 227, comparación: $/m² >28% bajo su grupo, factor 0.72) marca **CERO** — su mediana se calcula con `IN ('paralelo','oficial')` y **el tag `oficial` ya no existe**, así que la referencia quedó en 70 avisos de 761 y 537 no tienen contra qué compararse. **No falla: dejó de mirar.** Reparar la referencia es una línea, pero encender el badge de TC con ella miente en 13 de 15 casos (11 tienen precio redondo en dólares) → decisión pendiente en `docs/reports/AUDITORIA_SENALES_PRECIO_2026-08-19.md`. 🟢 **CORREGIDO 18-ago-2026: el badge SÍ dispara en el régimen nuevo — 93 de 656 props del feed, el 14%.** Esta fila decía "en Equipetrol/shadow el badge NO dispara y está bien así", y quedó vieja cuando la **mig 311** lo reactivó para el tag `oficial_viejo`. 🔑 **Es la mitad B de una decisión**: `precio_normalizado_shadow` **ya NO descuenta** el `oficial_viejo` — que el aviso mencione el rate viejo no prueba que el precio en dólares salga de ahí, y medido contra las vecinas del mismo edificio vale igual sin descontar —, así que **se publica lo que dice el anuncio y se enciende el badge**. No se inventa un descuento: se declara la duda. Ejemplo vivo: **8000865 (One Isuto)**, $95.400 / 53 m² = $1.800/m² con `tc_sospechoso = true`. En ZN/casas (régimen viejo) sigue disparando por el criterio de la 227. |
...
| `/admin/contactos` | **CRM B2C del bot** — quién le escribió a Simón por WhatsApp, con su conversación completa, sus selecciones (props + ♥ favoritos + aperturas) e intentos de contacto. Los datos entran SOLOS por `/api/kapso/webhook` (mig 292); la vista `v_simon_contactos_resumen` deriva los contadores (migs 296/300/301). APIs `/api/admin/contactos*`. 🔴 **La identidad YA NO es el teléfono (migs 318/319):** Meta lo está sacando del payload —quien adopta un username obtiene privacidad de número y `wa_id`/`from` desaparecen— y ahora es el par **(`meta_portfolio_id`, `business_scoped_user_id`)**, resuelto SIEMPRE por `simon_resolver_contacto()` con el orden **BSUID → teléfono → crear** (al revés se generan duplicados imposibles de fusionar). El BSUID **cambia con el tiempo** (el número del founder tiene 3, el último desde la reconexión del 28-jul): los viejos viven como alias en `simon_contacto_bsuids`. Lo que el ingest no entiende va crudo a `simon_eventos_sin_procesar` — Meta no reenvía. ⚠️ Las shortlists/favoritos siguen cruzando **por teléfono**: un contacto sin número los muestra en cero y la UI lo declara. Manual: `scripts/kapso-bsuid/README.md` |
...
**Rediseño mobile feeds (7-jul, EN PROD — `feat/mobile-feed-redesign` mergeado)**: ambos feeds (venta+alquiler) comparten el mismo patrón. Ventas = styled-jsx inline (`mc-*`/`mfh-*`/`mt-*`); alquileres = CSS externo `styles/alquileres.css` (`amc-*`/`mfh-*`/`mt-*`). Piezas: **header sticky** (`mfh-*`: logo+perfil+hamburguesa; 2da fila buscador nativo + botón filtros); **buscador inteligente** (`lib/busqueda-natural`, $0 sin IA) también en sidebar desktop (`dsk-search`); **card limpia** (corazón dentro de la foto, tap abre el sheet, sin acciones/descripción — todo lo transaccional en el sheet); **barra fija inferior** (`mt-bottombar`: Ver mapa + comparar 0/1/2+ favoritos + limpiar). **El mapa mobile explora por zona (3-ago, PR #62)**: a pantalla completa lleva un **carrusel de mini-tarjetas horizontales** (`mt-rail*`/`mt-rc*` · `alq-rail*`/`alq-rc*`) con lo que se ve, que **se actualiza solo** al mover el mapa — sin botón de confirmar, decisión de producto: en el celular el resultado se ve al instante. Sincronizado en dos sentidos con los pines (deslizar → resalta el pin; tocar un pin → desliza a su tarjeta). Botón puente **"Ver los N de esta zona"** (`mt-map-apply`/`alq-map-apply`) cierra el mapa y deja el feed acotado con chip `mt-area-chip`/`alq-area-chip` en el header. 🔴 Tope **`RAIL_MAX = 30`** tarjetas **declarado en el contador** ("372 en pantalla · las 30 más cercanas") — truncar en silencio se leería como "esto es todo". En alquileres el widget de chat **se oculta con el mapa abierto** (tapaba el contador). Regla layout: los hijos del contenido de la card usan `flex-shrink:0` (sin esto, en viewports bajos tipo iPhone SE el flex aplasta el título y lo recorta). **Menú hamburguesa** (`mfd-*`): Preventa (`/ventas?preventa=1`, lector en ventas aplica filtro) · Ventas · Alquileres · *Simulá y calculá* → Comparador de propiedades (abre CompareSheet con 2+ favs) + **Calculadora de renta `[Próximamente]`** + **Crédito hipotecario `[Próximamente]`** · Mercado · Mis favoritos · Hablar por WhatsApp (`SIMON_WHATSAPP`, número de negocio, NO el del fundador). Perfil (`mfp-*`): sin login, "guardá favoritos en este dispositivo". **Pendiente**: isologo oficial (el header usa placeholder arena+punto verde).
