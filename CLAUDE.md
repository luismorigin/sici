...
| **Estado de obra inferido (feed venta)** | Vista `v_estado_obra_inferido_shadow` (migs 302/303) — "sin confirmar" del **52% al 14%**. Dos señales validadas con backtest: **vecinos unánimes del edificio 96,7%** · **hay alquiler activo 95%** (no se alquila lo no construido). 🔴 **NO usar `proyectos_master.estado_construccion`: acierta 78%** (envejece). Se calcula **al leer** → una prop nueva sale inferida sola y el feed se corrige solo al entregarse el edificio. Expone `estado_origen` (aviso/vecinos/alquiler) para **declarar** lo deducido, nunca afirmarlo |
