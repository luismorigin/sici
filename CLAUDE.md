...
14. **Brokers — tres tablas distintas, NUNCA confundir**:
    ...
    - `broker_prospection` (237 + 238) — **tabla interna del founder** para outreach a captadores de Equipetrol (NO son usuarios de Simón, son agentes de Remax/C21/BI cuyo contacto está en `propiedades_v2.agente_telefono`). Agregada por la RPC `populate_broker_prospection()` desde `v_mercado_venta` agrupado por teléfono normalizado. Tier por volumen (T1: 1-5, T2: 6-10, T3: 11+). Tracking de 3 mensajes WA secuenciales. Admin en `/admin/prospection`. Lib `lib/broker-prospection.ts`.
    - Ninguna FK entre las 4. Datos completamente separados.
...
| **Demo público brokers** | Migración 236 + commits `74660a6`/`70dd916`. `/broker/demo` (Demo Broker, perspectiva profesional) + `/broker/demo/alquileres` + `/b/demo` (Demo Cliente, cómo recibe la shortlist). Sin login. Sanitización server-side de `agente_*` (lib/demo-mode.ts) + intercept de WA clicks → modal educativo. Banner top arena, intro sheet con cookie, watermark con CTA "Activá tu cuenta". Canal de prospección de brokers escalable (sin demo en vivo del founder). |
| **Prospección Brokers** | Migraciones 237+238. `/admin/prospection` con tabla `broker_prospection` (177 brokers captadores de Equipetrol). 3 mensajes WA secuenciales (Msg 1 con imagen + caption, Msg 2 bullets de valor, Msg 3 cierre 20 fundadores). 12 respuestas pre-armadas en drawer. Tiers T1/T2/T3 por volumen, orden dinámico por props/antigüedad. Lib `lib/broker-prospection.ts`. |
