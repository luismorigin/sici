...
**Deptos Equipetrol → HÍBRIDO** (en curso, rama `feat/deptos-hibrido-shadow`, NO merged): reemplazar el n8n de deptos-venta por el flujo híbrido (discovery propio + reader-extendido en sesión = MOAT + apply a entorno SHADOW aislado `propiedades_v2_shadow`, mig 268/269 + helper 271). Reader spec `scripts/deptos-equipetrol/READER_SPEC.md` (precio/TC/dorms/baños/piso/amenidades+extra/equipamiento-3-baldes/parqueo/baulera/estado/amoblado/equipado/multiproyecto). **TC nuevo** (unificación oficial=paralelo) congelado en `TC_NUEVO_DECISION.md` (default vs `oficial_viejo`; **principio de arquitectura: normalización = frontera de acceso, crudo+tag adentro / normalizado afuera** — portable, para la Plataforma Híbrida Genérica). Estado + pendientes (el comando end-to-end es lo grande): `scripts/deptos-equipetrol/SESION_READER_DISCOVERY.md` + memoria `project_checkpoint_deptos_hibrido`.
