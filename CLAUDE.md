...
- Tabla condominios (casas ZN): `condominios_master` (mig 260 tabla + mig 261 FK/matcher; **45 condominios cerrados curados**). Matcher areal `matchear_condominio(lat,lon,nombre)` (nombre-primario + GPS) + FK `id_condominio_master` en `propiedades_v2`. **305 casas ZN activas cargadas** (`tipo_propiedad_original='casa'`, marcadores `metodo_match` en `carga_piloto_casas_19jun/20jun`, `carga_casas_escala_20jun`, `carga_casas_nuevas_20jun`), **todas con contacto del captador (WhatsApp) en `datos_json_enrichment`**, aisladas del feed de deptos (0 en `v_mercado_venta`, 0 con `id_proyecto_master`). **Backfill 21-jun: paridad con el contrato de deptos** — las casas tienen `fotos_urls`+`cantidad_fotos`, `descripcion`, `fecha_publicacion`, `codigo_propiedad` (del portal), `estacionamientos`/`oficina_telefono` (solo C21; Remax no los expone), más `id` propio (ref pública `SIM-V<id>`) + `fuente` (Remax/C21) + `oficina_nombre`. Script reusable `scripts/auditoria-cola-matching/backfill-campos-casas.mjs` (función `extraerCampos()` para el cron futuro). Flujo de carga = **híbrido manual** (NO el n8n viejo, que es Equipetrol y no captura contacto): scripts en `scripts/sonda-suelo/` (discovery/dedup/fetch-contacto/merge) + agentes-lectores para el MOAT. Pendiente: feed `/ventas/casas` + cron (vista `v_mercado_casas` ✅ aplicada mig 262). Diseño: `docs/proyectos/zona-norte/DISENO_PIPELINE_CASAS_VIVIENDA.md`
...
15. **Skills `/audit-feed-ventas-mensual` y `/audit-feed-alquileres-mensual` — costo + confirmación**:
    ...
    - **`/audit-feed-ventas-semanal`** (v1.8, 2026-06-23): variante liviana sin Firecrawl, capas 2+3+4 sobre props nuevas en ventana temporal configurable (`--dias=N`, `--desde/--hasta`, `--ids`, `--macrozona`). **Costo $0**, sin persistencia. Source en `scripts/auditoria-feed-ventas/audit-feed-ventas-semanal.command.md`. Complementa la mensual: semanal limpia deuda fresca, mensual valida con drift portal. **v1.3 reescribió el detector de TC** (un solo check 2.1 = doble normalización paralelo, ratio `precio_usd/billete_desc` 1.2–1.6) tras un falso positivo que marcó `no_especificado` correctas como infladas; el gap oficial↔billete NUNCA se flaguea. **v1.6 (8-jun) alineó el scope con el feed público**: arg `--macrozona` (default `equipetrol`, = `ZONAS_EQUIPETROL_DB`) + filtro solo-departamentos — antes corría sobre `v_mercado_venta` completa y mezclaba Zona Norte + colaba terrenos (#2435); + check 2.4 sub-caso `tipo_operacion` mal clasificado (alquiler/anticrético colado en venta); + candado canónico **formato-objeto** (un string NO protege — el merge usa `_is_campo_bloqueado`). **v1.7 (17-jun) convirtió el check 3.1 (matching) de regex-juez a regex-filtro + juez LLM (nuevo paso 3.1b)**: los hits del 3.1 ya no se discriminan a ojo (generaba FP en clusters numerados) — se escalan a un agente-lector que lee el anuncio y da veredicto (OK / ALIAS_FALTANTE / MISMATCH / SIN_NOMBRE), mismo juez que `/audit-cola-matching`. **v1.8 (23-jun) agregó el check 2.9 — flag TC `paralelo` contradicho por el texto** (Capa 2 ahora 8 checks): props C21 con `precio_usd` correcto pero `tipo_cambio_detectado='paralelo'` mal → el feed las infla ×1.43; complementa al 2.1 (el 2.1 caza `precio_usd` mal ratio≈1.4, el 2.9 caza el flag mal ratio≈1.0). Usa la señal del portal (`BOB/monto`≈7=oficial / ≈10=paralelo) + agente-lector. Origen: bug TC histórico corregido en migraciones 263-265 (368 deptos Eq+ZN). Ver memoria `project_bug_tc_flag_paralelo_historico`. **⚠️ La copia precargada en `.claude/commands/` puede quedar desactualizada (está gitignored); la fuente de verdad es el `.command.md` del repo.** Ver `docs/arquitectura/TIPO_CAMBIO_SICI.md` + memorias `precio_paralelo_vs_oficial_billete.md` y `feedback_candado_formato_objeto.md`.
    - **`/audit-feed-alquileres-semanal`** (v1.5, 2026-06-08): equivalente alquileres. 7 checks capa 2 (sin TC paralelo, checks específicos amoblado/mascotas/expensas), 4 checks capa 3 (matching tokenizado + bandas GPS), 3 checks capa 4 (incluye drift recurrente histórico). 8 calibraciones aplicadas tras retest sobre 37 props. **Costo $0**. Source en `scripts/auditoria-feed-alquileres/audit-feed-alquileres-semanal.command.md`. Pre-requisito: cobertura cruda ≥70% (paso 0 gate). **v1.5 (8-jun, paridad con ventas v1.6)**: arg `--macrozona` (default `equipetrol`, = feed público) + candado **formato-objeto** (verificado: `merge_alquiler` y `registrar_*_alquiler` usan el mismo helper `_is_campo_bloqueado`). NO usa TC paralelo (precio `precio_mensual_bob` Bs/mes directo).
    - **`/audit-cola-matching`** (v0.1, 2026-06-19): audita la **cola** de matching (`matching_sugerencias.estado='pendiente_<macrozona>'`) ANTES de aprobar — distinta de las `/audit-feed-*` (que auditan el feed ya matcheado). Multi-macrozona (`--macrozona=zona-norte|equipetrol|urubo`; `equipetrol`→`estado='pendiente'`). Arquitectura: el `.mjs` (`scripts/auditoria-cola-matching/`) filtra/fetchea (lector C21 `?json=true`/Remax `data-page`)/ordena; el **veredicto lo dan subagentes-lectores (juez LLM)**, NUNCA el script. Genera SQL (UPDATE directo + candado `IS NULL`, nunca `aplicar_matches_aprobados`), read-only. Validada en producción 19-jun (cola ZN 33→0, Eq 17→10). Source `scripts/auditoria-cola-matching/audit-cola-matching.command.md`.
...
**⚠️ Este pipeline n8n es Equipetrol-only y NO captura el contacto del agente ni hace matching de condominios.** Las **305 casas de Zona Norte** (con WhatsApp del captador + condominios) se cargaron con un **flujo HÍBRIDO manual** (`scripts/sonda-suelo/`: discovery/dedup + fetch-contacto + agentes-lectores para el MOAT + matcher `matchear_condominio` + carga). **Las casas tienen el contrato completo de deptos** (fotos/descripción/fecha/código/contacto/MOAT) tras el backfill 21-jun (`scripts/auditoria-cola-matching/backfill-campos-casas.mjs`, función `extraerCampos()` reusable para el cron). **⚠️ BLINDAJE 21-jun (clave): deptos y casas comparten `propiedades_v2` → cada pipeline debe filtrar por TIPO. Los discovery de deptos ZN (`flujo_a_discovery_{remax,century21}_zonanorte`) ahora excluyen `tipo_propiedad_original IN ('casa','terreno','lote')` en "Obtener URLs Activas BD"/"Marcar Ausentes" (antes degradaban las casas remax ZN a `inactivo_pending`). Equipetrol NO se tocó. Los 3 workflows `casas_terrenos/` están DESACTIVADOS (prueba, los reemplaza el híbrido).** **Vista del feed: `v_mercado_casas` (`sql/migrations/262_v_mercado_casas.sql`, ✅ APLICADA 21-jun, 298 casas)** — espejo de `v_mercado_venta` con `tipo='casa'` + `es_activa` + 300/730d + JOIN condominios; devuelve ~298 casas. Plan a futuro: unificar todo bajo el híbrido + cron vía **routine de Claude Code** (MOAT sin API). Ver `docs/proyectos/zona-norte/DISENO_PIPELINE_CASAS_VIVIENDA.md`. Regla TC: "7" = oficial (no paralelo).
