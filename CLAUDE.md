...
| **Proyecto Zona Norte** | `docs/proyectos/zona-norte/` — **prototipo de la arquitectura multi-macrozona Simón Santa Cruz** (ADR-009). Fases 1-4 en producción 27-may-2026 + audit completo 28-may. **Estado 28-may:** 70 pm activos (100% con `gps_verificado_visual`), 212/391 props ZN venta matched (**54.2%**). Cleanup histórico K1+STONE+CURUPAU+Brickell (63 falsos), 11 GPS corregidos manualmente, Essenzia dividido, DOMUS MADERO + Sky Icon + Torre Baruc Norte descubiertos. Ticket #1 (prompt LLM) **ARCHIVADO** tras medir: el LLM no era el cuello de botella. Ticket #1.7 (detector automático clusters) creado para escalar sin sesiones manuales. **Nuevas columnas `proyectos_master`:** `gps_verificado_visual` (TEXT confirmed/sospechoso/dividir/mover), `gps_verificacion_notas`, `gps_verificado_visual_at`, `gps_verificado_osm` (BOOLEAN), `osm_buildings_around_30m`, `osm_nominatim_address`, `osm_verified_at`. **Herramienta reutilizable:** `scripts/verify-pm-gps/` (Overpass+Nominatim+HTML Leaflet con tile satelital Esri, $0, sin API key, lista para Urubó/Polanco). Workflows ZN son universales multi-macrozona. **Strangler pattern: Equipetrol producción NO se toca.** Empezar por README → DECISIONES.md (9 ADRs) → BACKLOG.md → BITACORA.md (cronología completa) → operacion.md (kill-switch). **Próximo:** #8 microzonas (bloquea #6) → #6 frontend `/mercado/zona-norte` → #1.7 detector. |
