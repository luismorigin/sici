...
| **⏳ Plan Contacto Directo B2C (bot)** | `docs/broker/CONTACTO_DIRECTO_B2C_PLAN.md` — **PLAN DOCUMENTADO, SIN IMPLEMENTAR** (validado en 3 rondas de review). Hace que las shortlists del bot `simon-asistente` (`/b/[hash]`) contacten al **captador** (`agente_telefono`/`agente_whatsapp`) en vez del broker dueño, vía flag `simon_brokers.contacto_directo` (solo ese broker; no afecta B2B). **Cuando se pida "implementar plan de contacto directo / broker B2C": checkout `git switch feat/shortlist-contacto-directo-b2c`, seguir §7, respetar advertencia §5 (leer cada guarda real, no aplicar la fórmula mecánicamente).** Migración `ADD COLUMN` + `UPDATE` la aplica el founder en Supabase. Gates = fase 2 (expiración larga). |
