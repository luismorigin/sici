...
├── components/feed/  → FeedDesktopNav (el ÚNICO componente realmente compartido entre feeds),
│                       EdificioSelect, IsotipoSimon, PriceHistogram
...
| `/zona-norte/ventas` + `/alquileres` | Feeds ZN `[dark launch/noindex]` — copia filtrando 14 microzonas ZN (`getMicrozonasZN()`), sin tocar Equipetrol. `lib/mercado-data-zn.ts` / `mercado-alquiler-data-zn.ts`. 🔴 **`/api/ventas` tiene EQUIPETROL COMO DEFAULT**: una llamada sin `zonas_permitidas` devuelve Equipetrol, **no un error**. El 18-ago eso hizo que el feed de ZN sirviera props de Eq. Centro y V. Brigida en producción — y **pasó `tsc` y `build` sin una queja**. Desde entonces `fetchFromAPI` de `zona-norte/ventas.tsx` **fuerza las zonas ZN** cuando no vienen, así el aislamiento no depende de que cada llamador se acuerde. ⚠️ Y el feed carga **24 en el SSG + el resto por fetch diferido**: si ese fetch no se dispara, muestra 24 de ~305 (pasó del 11 al 18-ago). Detalle: `docs/design/FIX_FEED_ZN_AISLAMIENTO.md` · memorias `project_feed_zona_norte_aislamiento` y `feedback_aislamiento_no_depende_del_llamador` |
