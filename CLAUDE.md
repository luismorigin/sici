...
> ⚠️ **Pre-cutover conviven DOS regímenes.** Lo de abajo describe el de **PROD** (`precio_normalizado()`, paralelo ×tc/6.96). El entorno **shadow** usa el régimen **TC-nuevo** (`precio_normalizado_shadow`, mig 272 — sin el ×1.47; ver `scripts/deptos-equipetrol/TC_NUEVO_DECISION.md`). **Ojo:** las **shortlists `/b/[hash]` leen shadow-first en PRODUCCIÓN** → ahí los precios ya salen con el régimen nuevo (~34% menores). El feed público (`/ventas`, `/alquileres`) sigue en prod (`?shadow=1` es opt-in). Al cutover el swap unifica todo.
