/**
 * Genera PDF del Estudio de Mercado Condado VI v2 con branding premium.
 ...
 *
 * Usage: npx puppeteer browsers install chrome && node scripts/generate-condado-pdf-v2.mjs
 */
...
const MD_PATH = resolve(__dirname, '../docs/reports/ESTUDIO_MERCADO_CONDADO_2026_03_v2.md')
const OUT_PATH = resolve(__dirname, '../docs/reports/ESTUDIO_MERCADO_CONDADO_2026_03_v2.pdf')
...
function mdToHtml(md) {
  ...
  function flushTable() {
    ...
    // Skip row 1 (separator), process data rows
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (hMatch) {
      ...
      result.push(`<h${level}>${formatInline(hMatch[2])}</h${level}>`)
      continue
    }
  ...
  // Fix: ordered lists used <ul> — swap back
  // Actually, we already differentiate. But numbered lists opened <ol> inside <ul> context.
  // Re-do: track list type properly. For simplicity, post-process:
  // (the logic above already handles ol vs ul correctly)
...
<head>
<meta charset="utf-8"/>
<title>Estudio de Mercado — Condado VI Plaza Italia (v2)</title>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Part headers (PARTE 1, PARTE 2, etc) — give them visual weight */
  h2:is([id]) {
    margin-top: 36px;
  }
</style>
...
<div class="cover">
  ...
  <div class="cover-meta">
    ...
    <strong>Fecha:</strong> 12 de marzo de 2026<br/>
    <strong>Fuente:</strong> SICI — 311 unidades verificadas con GPS<br/>
  ...
  await page.pdf({
    ...
    headerTemplate: `
      <div style="width:100%;font-family:'DM Sans',sans-serif;font-size:8px;color:#7A7168;display:flex;justify-content:space-between;padding:0 18mm;">
        <span>Condado VI Plaza Italia — Estudio de Mercado v2</span>
