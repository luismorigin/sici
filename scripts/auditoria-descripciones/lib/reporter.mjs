...
function renderSummary(resultados, runDir) {
  ...
  const lines = [];
  lines.push(`# Auditoría de descripciones — ${runDir.split(/[\\/]/).pop()}`);
  lines.push('');
  lines.push(`**Total props procesadas:** ${resultados.length}`);
  lines.push(`**Scraped OK:** ${ok.length} · **Fallaron scrape:** ${fallaron.length}`);
  lines.push('');
  ...
  if (conFlags.length > 0) {
    ...
    lines.push('');
    lines.push(
      '_Lista de props donde aparecieron/desaparecieron palabras críticas (sin filtro de similitud)._'
    );
    lines.push('');
