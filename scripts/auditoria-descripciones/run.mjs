...
async function main() {
  console.log(`▶ Auditoría de descripciones — limit=${LIMIT}, offset=${OFFSET}, concurrency=${CONCURRENCY}`);
  console.log(`  env: ${ENV_PATH || '(ninguno encontrado, usando env del proceso)'}`);

  if (!process.env.FIRECRAWL_API_KEY) {
    console.error('✖ FIRECRAWL_API_KEY no encontrada. Probé en:');
  ...
  console.log(
    onlyIds.length > 0
      ...
      : `▶ Buscando ${LIMIT} props más viejas en v_mercado_venta` +
          (excludeIds.length ? ` (excluyendo ${excludeIds.length} IDs de runs anteriores)` : '') +
  ...
  console.log(`▶ Scrapeando con Firecrawl (concurrencia ${CONCURRENCY})...`);
  let lastPct = -1;
  ...
  const okCount = scraped.filter((s) => s.ok).length;
  console.log(`  Scrape OK: ${okCount}/${scraped.length}`);

  console.log(`▶ Extrayendo descripciones y comparando...`);
  const resultados = scraped.map((s) => {
    if (!s.ok) {
      return {
        id: s.id,
        fuente: s.fuente,
...
function printResumen(rs) {
  ...
  console.log(`  ❌ Scrape fallidos:         ${rs.length - ok.length}`);
}
