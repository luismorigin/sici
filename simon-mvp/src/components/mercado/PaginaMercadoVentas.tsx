// Página de mercado de VENTA — compartida por todas las macrozonas.
// Las páginas (`/mercado/equipetrol/ventas`, `/mercado/zona-norte/ventas`) son
// envoltorios delgados: traen los datos de SU macrozona y los pasan acá.
// 🔑 Todo lo que varía por zona se DECLARA en `lib/macrozonas.ts`, no se escribe acá.
...
import type { MercadoData } from '@/lib/mercado-data'
import type { SerieMensual, VentasShadowExtra } from '@/lib/mercado-shadow-data'
...
export interface PaginaMercadoVentasProps {
  macrozona: Macrozona
  kpis: MercadoData['kpis']
  tipologias: MercadoData['tipologias']
  zonas: MercadoData['zonas']
  generatedAt: string
...
export default function PaginaMercadoVentas({
  macrozona,
  kpis,
  tipologias,
  zonas,
  ...
  extra,
  tcHoy,
}: PaginaMercadoVentasProps) {
  const { nombre, nombreSEO, rutaVenta, rutaMercado, indexable, geo } = macrozona
  const nZonas = zonas.length
  // Etiqueta legible de cada zona. `displayZona()` devuelve el `labelCorto` de
  // `zonas.ts`, que en Zona Norte está CIFRADO para tablas del admin ('ZN 2-3 Bz/Al').
  // La macrozona declara la versión que se lee ('2º-3º · Banzer/Alemana') — la misma
  // que ya usan los chips del feed. En Equipetrol las dos coinciden.
  // Se compara reproduciendo la MISMA transformación que aplicó el lib (`displayZona`),
  // no contra el labelCorto de la macrozona — ese ya viene reescrito y nunca matchearía.
  const labelZona = (display: string): string =>
    macrozona.zonasCanonicas.find(z => displayZona(z.db) === display)?.labelCorto ?? display
  const mesAnio = formatMesAnio(kpis.fechaActualizacion)
  // La caída en dólares se declara como BANDA (12-17%) SOLO donde la curva incluye el
  // tramo reexpresado: esa banda es el error del método de reexpresión (migs 287/288),
  // no una propiedad del mercado. Una macrozona cuya serie es toda medición directa no
  // tiene ese error — y darle la banda de otra zona sería inventarle una incertidumbre.
  const caidaUsd = serie
    ? serie.incluyeEstimado
      ? 'entre 12% y 17%'
      : `${Math.abs(serie.varUsdPct).toFixed(0)}%`
    : null
  // 🔑 En bolivianos el m² de Equipetrol volvió a donde empezó (19.768 → 19.692), y eso
  // redondeaba a "-0%" — un número que se lee como dato y es un artefacto del redondeo.
  // Además se estaba perdiendo la única frase que explica la brecha entre las dos monedas.
  const varBsRedondeada = serie ? Math.round(serie.varBsPct) : null
  const textoBs =
    varBsRedondeada == null ? null
    : varBsRedondeada === 0 ? 'en bolivianos está donde empezó'
    : varBsRedondeada < 0 ? `en bolivianos bajó ${Math.abs(varBsRedondeada)}%`
    : `en bolivianos subió ${varBsRedondeada}%`
  const fechaCorta = formatFechaCorta(kpis.fechaActualizacion)

  useEffect(() => {
    trackEvent('mercado_view', { operacion: 'venta', macrozona: macrozona.id })
  }, [])

  const title = `Precio del m² en ${nombre} hoy: $${kpis.medianaPrecioM2.toLocaleString('en-US')} USD — ${mesAnio} | Simon`
  // 🔑 El conteo de zonas se CUENTA (`nZonas`), no se escribe: decía "6 zonas" a mano
  // y Zona Norte tiene 14. Un número de más en la meta description no rompe nada — y
  // por eso nadie lo habría visto.
  const description = `Cuanto cuesta un departamento en ${nombreSEO}? Precio mediano del m²: $${kpis.medianaPrecioM2.toLocaleString('en-US')} USD. ${kpis.totalPropiedades} propiedades activas en ${nZonas} zonas. Datos actualizados ${fechaCorta}. Fuente: Simon Inteligencia Inmobiliaria.`
  const url = `https://simonbo.com${rutaMercado}/ventas`

  // FAQ visible = misma data que el FAQPage del schema (una sola fuente de copy)
  const faq: Array<{ q: string; a: string }> = [
    {
      q: `¿Cuánto cuesta el metro cuadrado en ${nombreSEO}?`,
      a: `El precio mediano del metro cuadrado en ${nombre} es de $${kpis.medianaPrecioM2.toLocaleString('en-US')} USD (${mesAnio}), calculado sobre ${kpis.totalPropiedades} departamentos activos${extra?.edificios ? ` en ${extra.edificios} edificios` : ''}. Es precio de publicación, no de cierre.`,
    },
    ...(serie
      ? [{
          q: `¿Los precios en ${nombre} están subiendo o bajando?`,
          a: `Medido en dólares, el m² bajó ${caidaUsd} desde ${serie.puntos[0].mes}${serie.incluyeEstimado ? ' (según el método de reexpresión)' : ''}. Medido ${textoBs}. La diferencia la explica el dólar paralelo, que subió ${serie.varTcPct.toFixed(0)}% en el mismo período: gran parte de la baja en USD es efecto cambiario, no caída del valor del inmueble.`,
        }]
      : []),
    ...(extra?.yieldZonas.length
      ? [{
          q: `¿Cuánto rinde un departamento en alquiler en ${nombre}?`,
      ...
      : []),
    {
      q: `¿Cuántos departamentos hay en venta en ${nombre}?`,
      a: `${kpis.totalPropiedades} departamentos activos en ${nZonas} zonas (${mesAnio})${extra?.domVenta ? `, con una mediana de ${extra.domVenta} días publicados` : ''}. Se verifican diariamente contra Century 21, Remax y Bien Inmuebles.`,
    },
  ]
    ...
    '@graph': [
      {
        ...
        description: `Plataforma de inteligencia de mercado inmobiliario en ${nombreSEO}. Monitoreo diario de precios, rotacion y tendencias.`,
      },
      {
        '@type': 'WebSite',
      ...
      },
      {
        ...
        datePublished: macrozona.mercadoDesde,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: nombre, item: `https://simonbo.com${rutaMercado}` },
      ...
      },
      {
        '@type': 'Dataset',
        '@id': `${url}#dataset`,
        name: `Precios de departamentos en ${nombreSEO} — ${mesAnio}`,
        description: `Analisis del mercado inmobiliario de ${nombre} con datos de ${kpis.totalPropiedades} departamentos en venta. Precio mediano del metro cuadrado: $${kpis.medianaPrecioM2} USD. Incluye serie historica de precios en USD y bolivianos desde enero 2026, rendimiento bruto de alquiler por zona y rotacion del inventario. Actualizado diariamente desde Century 21, Remax y Bien Inmuebles.`,
        url,
        ...
        variableMeasured: [
          { '@type': 'PropertyValue', name: `Precio mediano por metro cuadrado en ${nombre}`, value: kpis.medianaPrecioM2, unitText: 'USD/m2' },
          { '@type': 'PropertyValue', name: `Departamentos en venta en ${nombre}`, value: kpis.totalPropiedades, unitText: 'unidades' },
          ...
          ...tipologias.map(t => ({
            '@type': 'PropertyValue',
            name: `Precio mediano departamento ${DORM_LABELS[t.dormitorios] || t.dormitorios + 'D'} en ${nombre}`,
      ...
      },
      {
        ...
        mainEntity: faq.map(f => ({
          '@type': 'Question',
          name: f.q,
          acceptedAnswer: { '@type': 'Answer', text: `${f.a} Fuente: simonbo.com${rutaMercado}/ventas.` },
        })),
      },
    ],
  }
  ...
  const drillItems: TipologiaItem[] = tipologias.map(t => ({
    ...
    href: `${rutaVenta}?dormitorios=${t.dormitorios}`,
    hrefLabel: `Ver los ${t.unidades} →`,
  }))

  // Lecturas calculadas de la data viva (nunca números hardcodeados — doc-rot)
  // 🔴 Las lecturas se calculan sobre las MISMAS zonas que se dibujan.
  // Antes se calculaban sobre `zonas` entero mientras las barras filtran `>= 5 avisos`:
  // en Zona Norte eso hacía que el texto dijera "ZN 3-4 Al/Mu cuesta −40%" señalando una
  // barra que no estaba en pantalla (7 avisos repartidos, quedó fuera del corte).
  // En Equipetrol nunca se vio porque sus 6 zonas siempre superan el umbral.
  const MIN_AVISOS_ZONA = 5
  const zonasVisibles = zonas.filter(z => z.unidades >= MIN_AVISOS_ZONA)
  const zonasOrd = [...zonasVisibles].sort((a, b) => b.medianaPrecioM2 - a.medianaPrecioM2)
  ...
  const zMasOferta = [...zonasVisibles].sort((a, b) => b.unidades - a.unidades)[0]
  ...
  return (
    <>
      <Head>
        ...
        {/* ZN esta en dark launch: `indexable: false` la deja fuera de Google. Se declara
            desde la macrozona para que lanzar una zona sea cambiar UN booleano. */}
        <meta name="robots" content={indexable ? 'index, follow, max-snippet:-1, max-image-preview:large' : 'noindex, nofollow'} />
        ...
        <article className="mkt-wrap">
          <nav className="mkt-nav" aria-label="Breadcrumb">
            <Link href="/" className="mkt-brand">
              ...
              Simon
              <span className="mkt-crumb">Mercado · {nombre}</span>
            </Link>
          ...
          <div className="mkt-op" role="navigation" aria-label="Operación">
            ...
            <Link href={`${rutaMercado}/alquileres`} className="mkt-op-off">Alquilar</Link>
          </div>
          ...
          <header className="mkt-hero">
            ...
            <p className="mkt-lead">
              {serie
                ? <>El precio <b>viene bajando desde {serie.puntos[0].mes}</b>: {caidaUsd} en dólares — pero {textoBs}. La diferencia entre las dos monedas es el dólar.</>
                : <>{kpis.totalPropiedades} departamentos verificados diariamente en las {nZonas} zonas de {nombre}. Fuente: Century 21, Remax y Bien Inmuebles.</>}
            </p>
          </header>
          ...
          <div className="mkt-cards">
            ...
            {/* Actividad de mercado: la tarjeta APARECE SOLA cuando el dato vuelva a ser
                publicable. Hoy `absorcionPct` es null a proposito — ver el comentario en
                `lib/mercado-data.ts`: la serie de prod quedo congelada el 27-jul y la viva
                mide sobre 3 dias de bajas registradas. Un hueco es mejor que un 1%. */}
          ...
          {serie && (
            <section className="mkt-sec" aria-labelledby="sec-evolucion">
              ...
              <p className="mkt-sub">
                {serie.puntos[0].mes} → {serie.puntos[serie.puntos.length - 1].mes} · 1 dormitorio (la tipología más ofertada) ·{' '}
                {serie.incluyeEstimado ? 'historia reexpresada al criterio actual + medición diaria' : 'medición diaria'}
              </p>
              ...
              <Lectura>
                ...
                sugiere la cifra en USD.{serie.incluyeEstimado
                  ? ' La magnitud exacta en dólares depende del método de reexpresión (entre 12% y 17%); la forma de la curva es robusta.'
                  : ''} Precio de publicación — el de cierre puede diferir.
              </Lectura>
            </section>
          )}
          ...
          <section className="mkt-sec" aria-labelledby="sec-tipologias">
            ...
            {zTop && zBot && brechaPct > 0 && (
              <Lectura>
                ...
                {labelZona(zTop.zonaDisplay)} a {fmt(zBot.medianaPrecioM2)} en {labelZona(zBot.zonaDisplay)} (−{brechaPct}%). Mirá tu zona
          ...
          <section className="mkt-sec" aria-labelledby="sec-zonas">
            ...
            <ZonasBars
              items={zonasVisibles.map(z => ({
                label: labelZona(z.zonaDisplay),
            ...
            {zTop && zBot && zMasOferta && (
              <Lectura>
                <b>{labelZona(zBot.zonaDisplay)} cuesta −{brechaPct}% que {labelZona(zTop.zonaDisplay)}</b> — la brecha más grande del
                mapa. {labelZona(zMasOferta.zonaDisplay)} concentra el {concentracionPct}% de la oferta.
              </Lectura>
            )}
          </section>
          ...
          <footer className="mkt-foot">
            ...
            <div className="mkt-links">
              <Link href={`${rutaMercado}/alquileres`}>Ver mercado de alquileres</Link>
              <span aria-hidden="true">·</span>
              <Link href={rutaMercado!}>Volver al índice</Link>
            </div>
          </footer>
        </article>

        <CtaSticky href={rutaVenta} label={`Ver los ${kpis.totalPropiedades} departamentos →`} operacion="venta" />
