// Hub del mercado — compartido por todas las macrozonas (rediseño 21-jul-2026,
// "dos puertas"). Las páginas son envoltorios; lo que varía se declara en
// `lib/macrozonas.ts`. Los datos los arma `getPropsMercadoHub(macrozona)`, que se
// exporta desde acá para que cada página no duplique la lógica de los textos.
...
export type PaginaMercadoHubProps = Awaited<ReturnType<typeof getPropsMercadoHub>> & { macrozona: Macrozona }

export default function PaginaMercadoHub({
  macrozona,
  venta,
  alquiler,
  fechaActualizacion,
  ...
  copy,
  tcHoy,
}: PaginaMercadoHubProps) {
  const { nombre, nombreSEO, rutaMercado, indexable, geo } = macrozona
  const mesAnio = formatMesAnio(fechaActualizacion)
  const fechaCorta = formatFechaCorta(fechaActualizacion)
  const url = `https://simonbo.com${rutaMercado}`

  const title = `Mercado Inmobiliario ${nombre}: Ventas y Alquileres — ${mesAnio} | Simon`
  const description = `Inteligencia de mercado inmobiliario en ${nombreSEO}. Ventas: ${venta.total} departamentos, $${venta.m2}/m2. Alquileres: ${alquiler.total} unidades, Bs ${alquiler.rentaBs}/mes. Datos actualizados ${fechaCorta}.`
    ...
    '@graph': [
      {
        ...
        description: `Plataforma de inteligencia de mercado inmobiliario en ${nombreSEO}.`,
      },
      {
        '@type': 'WebSite',
      ...
      },
      {
        ...
        inLanguage: 'es',
        hasPart: [
          { '@type': 'WebPage', '@id': `https://simonbo.com${rutaMercado}/ventas`, name: `Mercado de Ventas en ${nombre}` },
          { '@type': 'WebPage', '@id': `https://simonbo.com${rutaMercado}/alquileres`, name: `Mercado de Alquileres en ${nombre}` },
        ],
        breadcrumb: {
          '@type': 'BreadcrumbList',
          itemListElement: [
      ...
      },
      {
        ...
        mainEntity: [
          {
            '@type': 'Question',
            name: `Cuanto cuesta un departamento en ${nombreSEO}?`,
            acceptedAnswer: {
              '@type': 'Answer',
              text: `El precio mediano del metro cuadrado en ${nombre} es de $${venta.m2.toLocaleString('en-US')} USD (${mesAnio}), con ${venta.total} departamentos en venta. Para alquiler, la renta mediana es Bs ${alquiler.rentaBs.toLocaleString('es-BO')}/mes con ${alquiler.total} unidades disponibles. Fuente: simonbo.com${rutaMercado}.`,
            },
          },
          ...(copy.roiMin != null && copy.roiMax != null
            ? [{
                '@type': 'Question',
                name: `Cuanto rinde un departamento en alquiler en ${nombre}?`,
                acceptedAnswer: {
                  '@type': 'Answer',
        ...
        <div className="hub-wrap">
          <nav className="hub-nav hub-in" aria-label="Breadcrumb">
            <a href="/" className="hub-brand">
              ...
              Simon
              <span className="hub-crumb">Mercado · {nombre}</span>
            </a>
          ...
          <header className="hub-hero hub-in hub-d1">
            ...
            <h1 className="hub-h1">Lo que pide {nombre} hoy, y cómo llegó hasta acá</h1>
          ...
          <div className="hub-doors">
            <a href={`${rutaMercado}/ventas`} className="hub-card hub-card-dark hub-in hub-d2" aria-label={`Mercado de ventas en ${nombre}`}>
            ...
            <a href={`${rutaMercado}/alquileres`} className="hub-card hub-card-arena hub-in hub-d3" aria-label={`Mercado de alquileres en ${nombre}`}>
              <div className="hub-card-top">
...
async function _propsShape(): Promise<{
...
/** Arma los props del hub para UNA macrozona. La usan las páginas delgadas. */
export async function getPropsMercadoHub(macrozona: Macrozona): Promise<Awaited<ReturnType<typeof _propsShape>>> {
  ...
  // Etiqueta legible: en Zona Norte el `labelCorto` de `zonas.ts` está cifrado para
  // tablas del admin ('ZN 3-4 Bz/Al'). Acá se muestra el que declara la macrozona.
  const etiquetaZona = (display: string): string =>
  ...
  if (yz.length && ventaData.zonas.length && roiMin != null && roiMax != null) {
    ...
    yieldFrase =
      normZona(zonaMasBarata.zonaDisplay) === normZona(zonaTopYield.zona)
        ? `La zona más barata para comprar es hoy la que más rinde (${etiquetaZona(zonaTopYield.zona)}, ${roiMax.toFixed(1).replace('.', ',')}%).`
        : `El rendimiento va de ${roiMin.toFixed(1).replace('.', ',')}% a ${roiMax.toFixed(1).replace('.', ',')}% según la zona; el más alto hoy es ${etiquetaZona(zonaTopYield.zona)}.`
  }
  ...
  // Devuelve los PROPS pelados: el `revalidate` lo pone cada página.
  return {
