// Página de mercado de ALQUILER — compartida por todas las macrozonas.
// Las páginas son envoltorios delgados; todo lo que varía por zona se DECLARA
// en `lib/macrozonas.ts`. Gemela de `PaginaMercadoVentas.tsx`.
...
import type { MercadoAlquilerData } from '@/lib/mercado-alquiler-data'
import type { AlquilerShadowExtra } from '@/lib/mercado-shadow-data'
...
export interface PaginaMercadoAlquileresProps {
  macrozona: Macrozona
  kpis: MercadoAlquilerData['kpis']
  tipologias: MercadoAlquilerData['tipologias']
  zonas: MercadoAlquilerData['zonas']
  generatedAt: string
  extra: AlquilerShadowExtra | null
  tcHoy: number | null
}

export default function PaginaMercadoAlquileres({
  macrozona,
  kpis,
  tipologias,
  zonas,
  ...
  extra,
  tcHoy,
}: PaginaMercadoAlquileresProps) {
  const { nombre, nombreSEO, rutaAlquiler, rutaMercado, indexable, geo } = macrozona
  const nZonas = zonas.length
  // Etiqueta legible de la zona (gemela de la de PaginaMercadoVentas).
  ...
  const url = `https://simonbo.com${rutaMercado}/alquileres`
  ...
  const title = `Alquiler en ${nombre}: Bs ${kpis.rentaMedianaBs.toLocaleString('es-BO')}/mes mediana — ${mesAnio} | Simon`
  const description = `Cuanto cuesta alquilar un departamento en ${nombreSEO}? Renta mediana: Bs ${kpis.rentaMedianaBs.toLocaleString('es-BO')}/mes. ${kpis.totalUnidades} departamentos disponibles. Datos actualizados ${fechaCorta}. Fuente: Simon Inteligencia Inmobiliaria.`
  ...
  const faq: Array<{ q: string; a: string }> = [
    {
      q: `¿Cuánto cuesta alquilar un departamento en ${nombre}?`,
      a: `Bs ${kpis.rentaMedianaBs.toLocaleString('es-BO')} por mes de mediana (${mesAnio})${usdRef ? `, unos $${usdRef.toLocaleString('en-US')} al dólar de hoy` : ''}. ${tipologias.length ? tipologias.map(t => `${DORM_LABELS[t.dormitorios] || t.dormitorios + 'D'}: ${fmtBs(t.rentaMedianaBs)}`).join(' · ') + '.' : ''} El alquiler en ${nombre} se publica y se paga en bolivianos.`,
    },
    {
    ...
    ...(extra?.domAlquiler && extra?.domVenta
      ? [{
          q: `¿Cuánto tarda en alquilarse un departamento en ${nombre}?`,
    ...
    '@graph': [
      {
        ...
        description: `Plataforma de inteligencia de mercado inmobiliario en ${nombreSEO}. Monitoreo diario de precios de venta y alquiler.`,
      },
      {
        '@type': 'WebSite',
      ...
      },
      {
        '@type': 'Dataset',
        '@id': `${url}#dataset`,
        name: `Rentas de departamentos en alquiler en ${nombreSEO} — ${mesAnio}`,
        description: `Analisis del mercado de alquiler de ${nombre} con ${kpis.totalUnidades} departamentos activos. Renta mediana: Bs ${kpis.rentaMedianaBs}/mes. Incluye cortes por amoblado (por tipologia), equipado y parqueo con su tamano de muestra, y rotacion del inventario. Actualizado diariamente.`,
        url,
        ...
        variableMeasured: [
          { '@type': 'PropertyValue', name: `Renta mensual mediana en ${nombre}`, value: kpis.rentaMedianaBs, unitText: 'BOB/mes' },
          { '@type': 'PropertyValue', name: `Departamentos en alquiler en ${nombre}`, value: kpis.totalUnidades, unitText: 'unidades' },
          ...
          ...tipologias.map(t => ({
            '@type': 'PropertyValue',
            name: `Renta mediana ${DORM_LABELS[t.dormitorios] || t.dormitorios + 'D'} en ${nombre}`,
            value: t.rentaMedianaBs,
            unitText: 'BOB/mes',
          })),
          ...splitTipos.map(t => ({
            '@type': 'PropertyValue',
            name: `Renta mediana ${DORM_LABELS[t.dormitorios] || t.dormitorios + 'D'} amoblado en ${nombre}`,
            value: (t.amobladoSi as NonNullable<typeof t.amobladoSi>).medianaBs,
            unitText: 'BOB/mes',
          })),
          ...zonas.map(z => ({
            '@type': 'PropertyValue',
            name: `Renta mediana en ${labelZona(z.zonaDisplay)}, ${nombre}`,
      ...
      },
      {
        ...
        mainEntity: faq.map(f => ({
          '@type': 'Question',
          name: f.q,
          acceptedAnswer: { '@type': 'Answer', text: `${f.a} Fuente: simonbo.com${rutaMercado}/alquileres.` },
        })),
      },
    ],
  }
  ...
  // Las lecturas se calculan sobre las MISMAS zonas que se dibujan (ver el comentario
  // gemelo en PaginaMercadoVentas: si no, el texto señala una barra que no está).
  const MIN_AVISOS_ZONA = 5
  const zonasVisibles = zonas.filter(z => z.unidades >= MIN_AVISOS_ZONA)
  const zonasOrd = [...zonasVisibles].sort((a, b) => b.rentaMedianaBs - a.rentaMedianaBs)
  ...
  return (
    <>
      ...
      <div className={`${figtree.variable} ${dmSans.variable} mka`}>
        <article className="mka-wrap">
          <nav className="mka-nav" aria-label="Breadcrumb">
            <Link href="/" className="mka-brand">
              ...
              Simon
              <span className="mka-crumb">Mercado · {nombre}</span>
            </Link>
          ...
          <div className="mka-op" role="navigation" aria-label="Operación">
            <Link href={`${rutaMercado}/ventas`} className="mka-op-off">Comprar</Link>
          ...
          <section className="mka-sec" aria-labelledby="sec-zonas">
            ...
            {zTop && zBot && zMasOferta && (
              <Lectura>
                <b>{labelZona(zMasOferta.zonaDisplay)} es la puerta de entrada</b>: la mayor oferta ({zMasOferta.unidades} avisos)
                {zMasOferta.zonaDisplay === zBot.zonaDisplay ? ' y la mediana más baja' : ''}. {labelZona(zTop.zonaDisplay)}, la más
          ...
          <footer className="mka-foot">
            ...
            <div className="mka-links">
              <Link href={`${rutaMercado}/ventas`}>Ver mercado de ventas</Link>
        ...
        <CtaSticky href={rutaAlquiler} label={`Ver los ${kpis.totalUnidades} alquileres →`} operacion="alquiler" />
