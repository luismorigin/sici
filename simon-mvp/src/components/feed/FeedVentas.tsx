// FeedVentas — el feed de departamentos en venta, PARAMETRIZADO POR MACROZONA.
//
// Movido desde `pages/ventas.tsx` el 18-ago-2026. NO se reescribio: se movio.
// Misma logica, mismo diseno, mismo tracking — ahora recibe `macrozona` en vez de
// asumir Equipetrol.
//
// PARA QUE: agregar una macrozona (Urubo, Las Palmas, Zona Este) pasa a ser declararla
// en `lib/macrozonas.ts` + una pagina delgada, en vez de copiar ~6.000 lineas. Y una
// mejora del feed llega a TODAS las zonas de una vez.
//
// Lo que NO vive aca, porque es propio de cada zona: el SEO (`VentasHead`) y el
// `getStaticProps`. Se quedan en cada pagina.
//
// [!] Un error aca llega a todas las zonas. Por eso `scripts/eval-feeds-zonas.mjs`
// mide TODAS en cada cambio, no solo la que se toca.
...
function buildEmptyMessage(f: FiltrosVentaSimple, zonasCanonicas: ZonaCanonica[]): string {
  const parts: string[] = []
  ...
  if (f.zonas_permitidas?.length) {
    const zonas = f.zonas_permitidas.map(z => {
      const found = zonasCanonicas.find(zc => zc.db === z)
...
async function fetchFromAPI(
  ...
  // devuelve Equipetrol, NO un error. Mientras este componente era solo de Equipetrol,
  // ese default coincidia con lo que se queria y el agujero no se veia. Al
  // parametrizarlo por macrozona, /zona-norte/ventas empezo a mostrar 358 props de
  // Equipetrol y 0 propias — lo detecto `scripts/eval-feeds-zonas.mjs` ANTES de
  // desplegar. Es el mismo bug que rompio produccion el 18-ago por la mañana.
  if (macrozona) {
    filtros = {
      ...filtros,
...
function FilterControls({ zonasCanonicas, minPrice, maxPrice, selectedDorms, selectedZonas, entrega, orden, proyecto, proyectoNames, onMinPrice, onMaxPrice, onToggleZona, onToggleDorm, onEntrega, onOrden, onProyecto, brokerMode = false, areaMin, areaMax, onAreaMin, onAreaMax, amenSel, onAmenToggle, priceValues }: {
  /** Zonas de ESTA macrozona para el filtro (ver lib/macrozonas.ts). */
...
// ===== Desktop Filters =====
function DesktopFilters({ zonasCanonicas, currentFilters, isFiltered, onApply, onReset, proyectoNames, brokerMode = false, areaMin, areaMax, onAreaMin, onAreaMax }: {
  /** Zonas de ESTA macrozona para el filtro (ver lib/macrozonas.ts). */
  zonasCanonicas: ZonaCanonica[]
  ...
  return (
    <div className="vf-wrap">
      <FilterControls zonasCanonicas={zonasCanonicas} minPrice={minPrice} maxPrice={maxPrice} selectedDorms={selectedDorms} selectedZonas={selectedZonas}
...
function FilterPillsVentas({ zonasCanonicas, currentFilters, isFiltered, onApply, onReset, proyectoNames, amenSel, onAmenToggle, priceValues }: {
  /** Zonas de ESTA macrozona para el filtro (ver lib/macrozonas.ts). */
  zonasCanonicas: ZonaCanonica[]
  ...
  const zonasLabel = selectedZonas.size === 0 ? 'Todas las zonas' : (() => {
    const arr = zonasCanonicas.filter(z => selectedZonas.has(z.db)).map(z => z.labelCorto)
...
const VentaCard = memo(function VentaCard({ nombreMacrozona, property: p, isFavorite, onToggleFavorite, onShare, onPhotoTap, onDetails, isFirst, brokerMode, onAddToShortlist, publicShareMode = false, contactoDirecto = false, brokerInfo = null, publicShareBroker = null, priceSnapshot = null, brokerComment = null, isDestacada = false, onReport, isReported = false }: {
  /** Nombre de la macrozona para el mensaje de WhatsApp del broker ("Trabajo en X"). */
        ...
        <div className="vc-actions">
          ...
          {publicShareMode && !contactoDirecto && publicShareBroker && (
            <a href={`https://wa.me/${publicShareBroker.telefono.replace(/\D/g, '')}?text=${encodeURIComponent(buildClientToBrokerMessage(p, publicShareBroker.nombre))}`}
              ...
              onClick={(e) => { e.preventDefault(); openWhatsApp(publicShareBroker.telefono, buildClientToBrokerMessage(p, publicShareBroker.nombre), { origen: 'card_public_share', propiedad_id: p.id, tipo_operacion: 'venta' }) }}>
          ...
          {(!publicShareMode || contactoDirecto) && p.agente_telefono && (
            <a href={`https://wa.me/${p.agente_telefono.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(buildAgentWaMessage(p, brokerInfo, nombreMacrozona))}`}
              target="_blank" rel="noopener noreferrer" className="vc-act-btn vc-act-wsp"
              onClick={(e) => {
                e.preventDefault()
                ...
                openWhatsApp(p.agente_telefono!, buildAgentWaMessage(p, brokerInfo, nombreMacrozona), { origen: 'card', propiedad_id: p.id, tipo_operacion: 'venta' })
              }}>
              <svg viewBox="0 0 24 24" width="14" height="14" fill="#1EA952"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              Whatsapp
            </a>
...
function MobileFilterCard({ zonasCanonicas, totalCount, filteredCount, isFiltered, onApply, onReset, proyectoNames, nombreMacrozona }: {
  /** Zonas de ESTA macrozona para el filtro (ver lib/macrozonas.ts). */
  zonasCanonicas: ZonaCanonica[]

  /** Nombre de la macrozona para el subtitulo ("departamentos en Zona Norte"). */
  nombreMacrozona: string;
  ...
  return (
    <div className="mfc">
      <div className="mfc-header">
        ...
        <span className="mfc-sub">{isFiltered ? `de ${totalCount} departamentos` : `departamentos en ${nombreMacrozona}`}</span>
      </div>
...
function FilterOverlay({ macrozona, isOpen, onClose, totalCount, filteredCount, isFiltered, onApply, onReset, proyectoNames, brokerMode = false, areaMin, areaMax, onAreaMin, onAreaMax, amenSel, onAmenToggle, priceValues }: {
  /** La macrozona del feed: sin esto el contador del overlay contaria Equipetrol. */
  macrozona: Macrozona
  ...
  useEffect(() => {
    ...
    previewRef.current = setTimeout(async () => {
      try {
        const result = await fetchFromAPI(currentFilters, undefined, macrozona)
  ...
  function handleBusqueda(texto: string) {
    ...
    const dbs = sig.zonas.map(slug => macrozona.zonasCanonicas.find(z => z.slug === slug)?.db).filter(Boolean) as string[]
  ...
  return (
    <div className="fo-overlay">
      ...
      <div className="fo-body">
        ...
        <FilterControls zonasCanonicas={macrozona.zonasCanonicas} minPrice={minPrice} maxPrice={maxPrice} selectedDorms={selectedDorms} selectedZonas={selectedZonas}
          entrega={entrega} orden={orden} proyecto={proyecto} proyectoNames={proyectoNames} onMinPrice={handleMinPrice} onMaxPrice={handleMaxPrice}
          onToggleZona={toggleZona} onToggleDorm={toggleDorm} onEntrega={v => setEntrega(v)} onOrden={v => setOrden(v)} onProyecto={v => setProyecto(v)}
          brokerMode={brokerMode} areaMin={areaMin} areaMax={areaMax} onAreaMin={onAreaMin} onAreaMax={onAreaMax}
...
function BottomSheet({ nombreMacrozona, property: p, isOpen, onClose, onShare, onCompare, isFavorite, onToggleFavorite, gateCompleted, onGate, isDesktop, properties, onSwapProperty, brokerMode = false, onAddToShortlist, publicShareBroker = null, contactoDirecto = false, brokerInfo = null, brokerComment = null, sideMode = false }: {
  /** Nombre de la macrozona para el mensaje de WhatsApp del broker ("Trabajo en X"). */
  nombreMacrozona: string
            ...
            {publicShareMode && !contactoDirecto && publicShareBroker ? (
              ...
              const buildSheetMsg = (): string => {
                ...
                if (brokerMode && brokerInfo) {
                  ...
                  let msg = `Hola, soy ${identidad}. Trabajo en ${nombreMacrozona}.\n\n`
              ...
              <a href={`https://wa.me/${p.agente_telefono.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(buildSheetMsg())}`}
                ...
                onClick={(e) => {
                  ...
                  openWhatsApp(p.agente_telefono!, buildSheetMsg(), { origen: 'sheet', propiedad_id: p.id, tipo_operacion: 'venta' })
                }}>
                <svg viewBox="0 0 24 24" width="16" height="16" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
...
// ===== Page =====
export default function FeedVentas({ macrozona, head, seo, initialProperties = [], brokerSlug: brokerSlugProp = null, broker: brokerProp = null, publicShare = null, brokerDemoMode = false }: {
  /** La macrozona que este feed muestra. OBLIGATORIA: sin default, para que
   *  ninguna pagina sirva Equipetrol por descuido (incidente del 18-ago-2026). */
  macrozona: Macrozona;
  /** El <Head> de la pagina: SEO propio de cada macrozona. */
  head?: React.ReactNode;
  seo: VentasSEO; initialProperties: UnidadVenta[]; brokerSlug?: string | null; broker?: Broker | null; publicShare?: PublicShareData | null; brokerDemoMode?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (typeof q.zonas === 'string') {
      q.zonas.split(',').map(s => s.trim().toLowerCase()).forEach(slug => {
        ...
        const db = macrozona.zonasCanonicas.find(z => z.slug === slug)?.db
  ...
  useEffect(() => {
    ...
    let cancelled = false
    fetchFromAPI({ solo_con_fotos: false }, spotlightId, macrozona).then(({ spotlight }) => {
  ...
  const searchPillText = useMemo(() => {
    ...
    if (filters.zonas_permitidas?.length) {
      const zonas = filters.zonas_permitidas.map(z => {
        const found = macrozona.zonasCanonicas.find(zc => zc.db === z)
        return found ? found.labelCorto : z
      })
  ...
  const fetchProperties = useCallback(async (f?: FiltrosVentaSimple) => {
    ...
    try {
      const result = await fetchFromAPI(f || filters, undefined, macrozona)
  ...
  const menuDrawer = menuOpen && (
    <div className="mfd-scrim" onClick={() => setMenuOpen(false)}>
      <nav className="mfd" onClick={e => e.stopPropagation()} aria-label="Menú principal">
        ...
        <a className="mfd-item" href={macrozona.rutaAlquiler}>Alquileres</a>
  ...
  return (
    <>
      {/* El <Head> (SEO) es PROPIO de cada macrozona: titulo, description y el
          Schema.org con sus FAQ nombran la zona. Por eso no vive acá — cada
          pagina pasa el suyo por este slot. Ver `lib/macrozonas.ts`. */}
      {head}
      ...
      {!splitDesktop && <BottomSheet nombreMacrozona={macrozona.nombre} property={sheetProperty} isOpen={sheetOpen}
      ...
      {splitDesktop && sheetOpen && sheetProperty && (
        <BottomSheet nombreMacrozona={macrozona.nombre} property={sheetProperty} isOpen sideMode
          onClose={() => { setSheetOpen(false); setSheetProperty(null) }}
      ...
      {brokerMode && broker && (
        <div className="vt-broker-banner">
          ...
          {macrozona.rutaMercado && <a
            ...
            href={macrozona.rutaMercado}
            target="_blank"
            rel="noopener"
            onClick={() => trackEvent('broker_open_mercado', { broker_slug: broker?.slug })}
            title={`Abrir dashboard de mercado de ${macrozona.nombre} en pestaña nueva`}
          >
      ...
      <FilterOverlay macrozona={macrozona} isOpen={filterOverlayOpen} onClose={() => setFilterOverlayOpen(false)}
      ...
      {(isDesktop || publicShareMode || brokerMode) ? (
        ...
        <div className={`ventas-desktop ${splitDesktop ? 'ventas-desktop-split' : ''} ${publicShareMode ? 'ventas-desktop-public' : ''} ${shortlistRedesign ? 'vt-shortlist-redesign' : ''} ${(brokerMode && !isDesktop) ? 'ventas-desktop-broker-mobile' : ''} ${brokerMode ? 'ventas-desktop-broker' : ''}`}>
          ...
          {splitDesktop && (
            <FeedDesktopNav macrozona={macrozona} active="ventas" variant="dark"
          ...
          {!splitDesktop && !publicShareMode && !(brokerMode && !isDesktop) && (
            <aside className="ventas-sidebar">
              ...
              <div className="ventas-sidebar-count">
                ...
                <span className="ventas-count-text">{isFiltered ? `de ${unfilteredCount} departamentos` : `departamentos en ${macrozona.nombre}`}</span>
              </div>
              ...
              <DesktopFilters zonasCanonicas={macrozona.zonasCanonicas} key={`df-${filterComponentVersion}`} currentFilters={filters} isFiltered={isFiltered} onApply={applyFilters} onReset={resetFilters} proyectoNames={proyectoNames}
                brokerMode={brokerMode} areaMin={areaMin} areaMax={areaMax}
                onAreaMin={setAreaMin}
                onAreaMax={setAreaMax} />
            </aside>
          ...
          <main className="ventas-main">
            ...
            {!splitDesktop && !loading && properties.length === 0 && !loadError && <div className="ventas-status">{buildEmptyMessage(filters, macrozona.zonasCanonicas)}</div>}
            ...
            {!splitDesktop && spotlightProperty && (
              <div className="ds-spotlight">
                ...
                <VentaCard nombreMacrozona={macrozona.nombre} property={spotlightProperty} isFavorite={favorites.has(spotlightProperty.id)}
              ...
              <div className={`vd-cols ${listOnly ? 'vd-cols-solo' : ''}`}>
                <div className="vd-left">
                  ...
                  <div className="dsk-search vd-search">
                    ...
                    {natAviso === 'alquiler' && <a className="dsk-search-aviso dsk-search-link" href={macrozona.rutaAlquiler}>Parece que buscás alquilar → Ver alquileres</a>}
                    {/* Módulo guiado: pills sugeridas mientras no hay búsqueda ni filtros */}
                    {!isFiltered && !natQuery && (
                      <div className="dsk-pills">
                        {macrozona.ejemplosBusqueda.map(s => (
                  ...
                  <FilterPillsVentas zonasCanonicas={macrozona.zonasCanonicas} key={`fp-${filterComponentVersion}`} currentFilters={filters} isFiltered={isFiltered}
                ...
                <div className="vd-list">
                  ...
                  {!loading && displayedProperties.length === 0 && <div className="ventas-status" style={{ minHeight: 160 }}>{buildEmptyMessage(filters, macrozona.zonasCanonicas)}</div>}
            ...
            {!splitDesktop && displayedProperties.length > 0 && viewMode === 'grid' && (
              <div className="ventas-grid">
                ...
                {(spotlightProperty ? displayedProperties.filter(p => p.id !== spotlightId) : displayedProperties).map((p, idx) => (
                  <VentaCard nombreMacrozona={macrozona.nombre} key={p.id} property={p} isFavorite={favorites.has(p.id)} isFirst={idx === 0}
                    onToggleFavorite={onCardToggleFavorite} onShare={onCardShare}
                    onPhotoTap={onCardPhotoTap} onDetails={onCardOpenSheet}
        ...
        <main style={{ ['--mfh-h' as any]: 'calc(104px + env(safe-area-inset-top))' }}>
          ...
          <header className="mfh">
            ...
            {(natChips.length > 0 || natAviso || mapArea) && (
              <div className="mfh-under">
                ...
                {natAviso === 'alquiler' && <a className="mfh-aviso mfh-aviso-link" href={macrozona.rutaAlquiler}>Parece que buscás alquilar → Ver alquileres</a>}
              </div>
            )}
          </header>
          ...
          <div className={`mt-feed ${!brokerMode ? 'mt-feed-compare' : ''}`} ref={feedRef}>
            ...
            {!loading && properties.length === 0 && !loadError && <div className="mc" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7A7060', textAlign: 'center', padding: '0 32px' }}>{buildEmptyMessage(filters, macrozona.zonasCanonicas)}</div>}
