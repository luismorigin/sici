...
function buildEmptyMessage(f: FiltrosVentaSimple): string {
  ...
  if (f.zonas_permitidas?.length) {
    const zonas = f.zonas_permitidas.map(z => {
      const found = ZONAS_ZONA_NORTE.find(zc => zc.db === z)
      return found ? chipLabelZN(found.label) : z
    })
...
function buildFilters(
  ...
  // Blindaje ZN: el piso SIEMPRE son las microzonas Zona Norte. Si el usuario no
  // elige microzona, mandamos todas las ZN (nunca caer al default Equipetrol del API).
  f.zonas_permitidas = zonas.size > 0 ? [...zonas] : getMicrozonasZN()
        ...
        <div className="vf-zona-btns">
          {ZONAS_ZONA_NORTE.map(z => (
            <button key={z.db} className={`vf-zona-btn ${selectedZonas.has(z.db) ? 'active' : ''}`}
              title={z.label} onClick={() => onToggleZona(z.db)}>{chipLabelZN(z.label)}</button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="mfc">
      <div className="mfc-header">
        ...
        <span className="mfc-sub">{isFiltered ? `de ${totalCount} departamentos` : 'departamentos en Zona Norte'}</span>
      </div>
            ...
            {publicShareMode && !contactoDirecto && publicShareBroker ? (
              ...
              const buildSheetMsg = (): string => {
                ...
                if (brokerMode && brokerInfo) {
                  ...
                  let msg = `Hola, soy ${identidad}. Trabajo en Zona Norte.\n\n`
          ...
          {!publicShareMode && !(brokerMode && !isDesktop) && (
            <aside className="ventas-sidebar">
              ...
              <div className="ventas-sidebar-count">
                ...
                <span className="ventas-count-text">{isFiltered ? `de ${unfilteredCount} departamentos` : 'departamentos en Zona Norte'}</span>
              </div>
