...
// 🚪 ZONA NORTE (dark launch): el selector usa las 14 microzonas ZN con id = valor BD.
// El RPC acepta valores BD directos (igual que el feed Equipetrol con ZONAS_EQUIPETROL_DB),
// así que todas las referencias a ZONAS_ALQUILER_UI de la página siguen funcionando sin tocarlas.
// Etiqueta legible SIN hover (feed mobile-first): se descarta el labelCorto cifrado
// ('ZN 2-3 LS/Bz') y se arma desde el label completo → '2º-3º · La Salle/Banzer'.
...
const ZONAS_ALQUILER_UI = ZONAS_ZONA_NORTE.map(z => ({ id: z.db, label: chipLabelZN(z.label), titulo: z.label }))
...
export default function AlquileresPage({
  ...
  const fetchProperties = useCallback(async (f: FiltrosAlquiler, retry = true): Promise<number> => {
    ...
    // 🛡️ BLINDAJE ZN: nunca mandar zonas vacío → el API caería al default Equipetrol.
    // Piso = las 14 microzonas ZN (el feed público filtra getMicrozonasZN()). Cubre
    // mount, reset, filtros y deep-links: este es el único punto que llama al API.
    f = { ...f, zonas_permitidas: f.zonas_permitidas?.length ? f.zonas_permitidas : getMicrozonasZN() }
        ...
        <div className={`desktop-layout ${publicShareMode ? 'desktop-layout-public' : ''} ${brokerMode ? 'desktop-layout-broker' : ''}`}>
          ...
          <aside className="desktop-sidebar" style={{ overscrollBehavior: 'contain' }}>
            ...
            <div className="desktop-sidebar-count">
              ...
              <span className="desktop-count-label">{isFiltered ? `de ${totalCount} alquileres` : 'alquileres en Zona Norte'}</span>
            </div>
            <DesktopFilters
                ...
                {showZonaBanner && (
                  <div className="utm-zona-banner">
                    <div className="utm-zona-text">
                      <span className="utm-zona-title">Zona Norte tiene 14 microzonas con precios distintos</span>
        ...
        <>
          ...
          {activeCardIndex === 0 && !loading && properties.length > 0 && (
            <div className="alq-context-badge">
              {properties.length} deptos en alquiler · Zona Norte
            </div>
          )}
