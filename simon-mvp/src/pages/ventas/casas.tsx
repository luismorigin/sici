...
function BSGallery({ photos, propertyId }: { photos: string[]; propertyId?: number }) {
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      if (idx > 0) trackEvent('swipe_photos', { property_id: propertyId, origen: 'bottom_sheet_casa' })
    }
        ...
        <div className="bs-sticky-footer">
          {p.agente_telefono && (
            <a href={`https://wa.me/${p.agente_telefono.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(buildCasaWaMsg(p))}`}
              ...
              onClick={(e) => {
                e.preventDefault()
                trackEvent('click_whatsapp_casa', { property_id: p.id, zona: displayZona(p.zona), precio_usd: Math.round(p.precio_norm), origen: 'detail_sheet' })
        ...
        <div className="vc-actions">
          ...
          {p.agente_telefono && (
            <a href={`https://wa.me/${p.agente_telefono.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(buildCasaWaMsg(p))}`}
              ...
              onClick={(e) => {
                e.preventDefault()
                trackEvent('click_whatsapp_casa', { property_id: p.id, zona: displayZona(p.zona), precio_usd: Math.round(p.precio_norm), origen: 'card_desktop' })
                openWhatsApp(p.agente_telefono!, buildCasaWaMsg(p))
              }}>
              <WspSVG /> Whatsapp
            </a>
        ...
        <div className="mc-actions">
          ...
          {p.agente_telefono && (
            <a href={`https://wa.me/${p.agente_telefono.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(buildCasaWaMsg(p))}`}
              ...
              onClick={(e) => {
                e.preventDefault()
                trackEvent('click_whatsapp_casa', { property_id: p.id, zona: displayZona(p.zona), precio_usd: Math.round(p.precio_norm), origen: 'card_mobile' })
                openWhatsApp(p.agente_telefono!, buildCasaWaMsg(p))
              }}>
