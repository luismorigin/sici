// API: Server-side proxy for buscar_unidades_simple_shadow (ENTORNO SHADOW — validación)
// Clon de /api/ventas apuntando a la RPC shadow (propiedades_v2_shadow + precio_normalizado_shadow),
// para VER el feed del híbrido de deptos en el front sin tocar prod. Dark-launch.
// 🔒 GUARDA: solo responde en desarrollo → 404 en producción (inerte si se deployara).
// POST /api/ventas-shadow { filtros: { ... }, spotlightId?: number }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // 🔒 Solo desarrollo: el feed shadow NO se sirve en producción.
  ...
  try {
    ...
    const dataResult = await supabase.rpc('buscar_unidades_simple_shadow', { p_filtros: rpcFiltros })
    ...
    if (spotlightId && typeof spotlightId === 'number' && !data.find((d: any) => d.id === spotlightId)) {
      try {
        const spotResult = await supabase.rpc('buscar_unidades_simple_shadow', {
