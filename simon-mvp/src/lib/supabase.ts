...
// ⛔ Las 3 búsquedas de venta del funnel premium se borraron el 14-ago-2026:
//   `buscarUnidadesReales()`  → RPC `buscar_unidades_reales` (régimen TC VIEJO)
//   `buscarUnidadesBroker()`  → RPC `buscar_unidades_broker`
//   `buscarUnidadesUnificadas()` → combinaba las dos
//
// Las dos últimas ya estaban muertas (ningún llamador en todo `src/`), y la primera solo
// la usaban `/resultados-v2` y `FilterBarPremium`, o sea el funnel — apagado en este mismo
// commit. El feed público NO pasaba por acá: usa `buscar_unidades_simple_shadow` vía
// `rpcShadowFirst`, y el ACM lee `v_mercado_venta_shadow` directo.
//
// 🔑 Por qué importaba borrarlas y no solo dejar de llamarlas: `buscar_unidades_reales`
// calcula con `precio_normalizado()` (régimen viejo, ~47% más alto) y figuraba como
// bloqueante del TIEMPO 2 del cutover. Mientras el import siguiera en el código, el próximo
// que hiciera un grep la volvería a contar como viva. Su único llamador restante es
// `/admin/propiedades`, que se muda con los pasos 2-3 del admin.
//
// El tipo `UnidadReal` SE QUEDA: lo usan otras funciones de este archivo.
// Recuperarlas: `git show 0324f92:simon-mvp/src/lib/supabase.ts` (commit anterior a éste).
...
// ⛔ `construirAnalisisDesdeBusqueda()` borrada el 14-ago-2026 con el funnel premium.
// Era la tercera de la cadena muerta: llamaba a `buscarUnidadesReales` ("usar la misma
// función de búsqueda que /resultados", decía su comentario) y NADIE la llamaba a ella.
// El typecheck la delató — un grep sobre `src/` no la encontraba porque su único llamador
// era interno a este archivo. Recuperarla: `git show 0324f92:simon-mvp/src/lib/supabase.ts`.
