/**
 * HTML Template for Informe Fiduciario Premium v2
 * Extracted from api/informe.ts — CSS + HTML generation
 */

import type { Propiedad, DatosUsuario, Analisis, LeadData } from './types'
import {
  fmt, getCategoria, getNegociacion, getDescuento,
  getAmenityEmoji, getTradeOffLabel, zonaDisplay,
  formatDormitorios, formatFechaEntrega, getCostosEstimados,
  calcularValorEquipamiento, calcularPrecioReal,
...
// Amenidades de edificio vs equipamiento del depto
...
export function generateInformeHTML(input: TemplateInput): string {
  const { propiedades, datosUsuario, analisis } = input
  ...
  // ===================== HTML — Part 1: Head + Hero + Quick Summary + Search + Sections 1-2 =====================
...
__SPLIT_MARKER__`
  ...
  // Return will be completed in the second half
  return html.replace('__SPLIT_MARKER__', generateSections3to9(
    fav, comp1, comp2, todas, top3,
    datosUsuario, analisis,
    fechaHoy, precioM2Promedio, favCat,
    necesitaParqueo, necesitaBaulera,
    datosFav, datosComp1, datosComp2,
    equipFav, equipComp1, equipComp2,
    barrasChartHTML
  ))
}

// ===================== HTML — Part 2: Sections 3-9 + Footer =====================
interface DatosComparativos {
  precioPublicado: number; precioReal: number; extrasNecesarios: string[]; costoExtras: number
  expensasMin: number; expensasMax: number; impacto5Anos: number; diasMercado: number
  rankingEdificio: number | null; unidadesEdificio: number | null
  amenidadesPedidas: number; amenidadesTiene: number; valorEquipamiento: number
  tieneParqueo: boolean; tieneBaulera: boolean
}
interface EquipData { equipamientoReal: string[]; allAmenities: string[] }

function generateSections3to9(
  fav: Propiedad, comp1: Propiedad | undefined, comp2: Propiedad | undefined,
  todas: Propiedad[], top3: Propiedad[],
  datosUsuario: DatosUsuario, analisis: Analisis,
  fechaHoy: string, precioM2Promedio: number,
  favCat: { clase: string; texto: string; pct: number; badgeClass: string },
  necesitaParqueo: boolean, necesitaBaulera: boolean,
  datosFav: DatosComparativos, datosComp1: DatosComparativos | null, datosComp2: DatosComparativos | null,
  equipFav: EquipData, equipComp1: EquipData | null, equipComp2: EquipData | null,
  barrasChartHTML: string
): string {
  return `
    ...
    ${generateSections7to9(fav, comp1, comp2, todas, datosUsuario, analisis, fechaHoy, precioM2Promedio, favCat, necesitaParqueo, necesitaBaulera, datosFav, datosComp1, datosComp2)}
`
}

// ===================== Sections 7-9 + Footer =====================
function generateSections7to9(
  fav: Propiedad, comp1: Propiedad | undefined, comp2: Propiedad | undefined,
  todas: Propiedad[],
  datosUsuario: DatosUsuario, analisis: Analisis,
  fechaHoy: string, precioM2Promedio: number,
        ...
        <div class="section">
            ...
            <div class="section-content">
                ...
                ${(() => {
                  ...
                  descuentoBase = Math.max(3, Math.min(descuentoBase, 15))
        ...
        <div class="section" style="background: #0a0a0a; border: 1px solid rgba(201,169,89,0.3);">
            ...
            <div class="section-content">
                ...
                <div style="display: grid; gap: 20px; ${comp1 || comp2 ? 'grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));' : ''}">
                    ${generateContactCard(fav, '#1 FAVORITA', 'var(--gold)', '#0a0a0a', '1', datosUsuario, necesitaParqueo, necesitaBaulera)}
                    ${comp1 ? generateContactCard(comp1, '#2', 'rgba(201,169,89,0.2)', 'var(--cream)', '2', datosUsuario, necesitaParqueo, necesitaBaulera) : ''}
                    ${comp2 ? generateContactCard(comp2, '#3', 'rgba(201,169,89,0.2)', 'var(--cream)', '3', datosUsuario, necesitaParqueo, necesitaBaulera) : ''}
                </div>
...
// Helper to generate contact card for each property
function generateContactCard(
  prop: Propiedad, label: string, badgeBg: string, badgeColor: string,
  posicion: string, datosUsuario: DatosUsuario,
  necesitaParqueo: boolean, necesitaBaulera: boolean
): string {
  const borderStyle = posicion === '1'
    ? 'background: rgba(201,169,89,0.1); border: 1px solid rgba(201,169,89,0.3);'
    : 'background: rgba(201,169,89,0.05); border: 1px solid rgba(201,169,89,0.2);'

  const badgeIcon = posicion === '1'
    ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="#0a0a0a" stroke="#0a0a0a" stroke-width="1"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>'
    : posicion === '2'
    ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="2"><circle cx="12" cy="8" r="6"/><path d="M12 14v8M8 22h8"/></svg>'
    : '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#cd7f32" stroke-width="2"><circle cx="12" cy="8" r="6"/><path d="M12 14v8M8 22h8"/></svg>'

  return `
                    <div style="${borderStyle} border-radius: 16px; padding: 20px;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
                            <span style="background: ${badgeBg}; color: ${badgeColor}; padding: 4px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px;">${badgeIcon} ${label}</span>
                        </div>
                        <h4 style="font-size: 1.1rem; margin-bottom: 5px; color: var(--cream);">${prop.proyecto}</h4>
                        <p style="opacity: 0.8; font-size: 0.9rem; margin-bottom: 15px; color: var(--cream);">$${fmt(prop.precio_usd)} · ${Math.round(prop.area_m2)}m²</p>
                        ${prop.asesor_wsp ? `
                            <p style="font-size: 0.85rem; opacity: 0.9; margin-bottom: 10px; color: var(--cream);">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="1.5" style="display: inline; vertical-align: middle;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> ${prop.asesor_nombre || 'Asesor'}${prop.asesor_inmobiliaria ? ` · ${prop.asesor_inmobiliaria}` : ''}
                            </p>
                            <button
                                data-broker='${JSON.stringify({propId:String(prop.id), posicion, proyecto:prop.proyecto, precio:String(prop.precio_usd), dormitorios:String(prop.dormitorios), broker:prop.asesor_nombre||'Asesor', brokerWsp:prop.asesor_wsp||'', inmobiliaria:prop.asesor_inmobiliaria||'', preguntas:JSON.stringify(generarPreguntasPersonalizadas(prop, datosUsuario, necesitaParqueo, necesitaBaulera))}).replace(/'/g, '&#39;')}'
