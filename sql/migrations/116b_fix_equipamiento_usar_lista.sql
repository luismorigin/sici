-- ============================================================================
-- MIGRACIÓN 116: Fix equipamiento_detectado usar equipamiento lista guardada
-- ============================================================================
-- Fecha: 2026-02-03
-- Propósito: Que equipamiento_detectado lea de datos_json->amenities->equipamiento
--            (igual que el editor) en vez de detectar desde descripción
-- Problema: El equipamiento guardado en el editor no aparecía en resultados
-- Depende de: Migración 115 (amenities_confirmados)
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    -- Antes: detectaba de descripción con ILIKE (no encontraba nada)
    -- Ahora: lee de datos_json->amenities->equipamiento (igual que editor)
    -- ========================================================================
    CASE
...
DO $$
DECLARE
  v_con_equipamiento INTEGER;
BEGIN
  SELECT COUNT(*) INTO v_con_equipamiento
  FROM propiedades_v2
  WHERE status = 'completado'
    AND duplicado_de IS NULL
    AND es_activa = true
    AND jsonb_typeof(datos_json->'amenities'->'equipamiento') = 'array'
    AND (datos_json->'amenities'->'equipamiento')::text != '[]';

  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE 'MIGRACIÓN 116: equipamiento_detectado usa lista guardada';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE 'Propiedades con equipamiento guardado: %', v_con_equipamiento;
  RAISE NOTICE 'Cambio: Lee de datos_json->amenities->equipamiento';
