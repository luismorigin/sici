-- =============================================================================
-- Migración 280 (SHADOW) — RPC feed VENTA: chip pet_friendly del EDIFICIO
-- -----------------------------------------------------------------------------
-- Gemela de mig 279 (alquiler). Sigue a mig 278 (proyectos_master.pet_friendly).
-- Re-crea buscar_unidades_simple_shadow con 2 cambios:
--   1. + columna `pet_friendly boolean` al final del RETURNS TABLE (de pm.pet_friendly;
--      ya se joinea proyectos_master). El front lo muestra como CHIP.
--   2. Saca "Pet Friendly" de la salida de amenidades (`amenities_lista` jsonb +
--      `amenities_confirmados`) → ahora es el chip dedicado, no una amenidad más.
-- Cambiar el RETURNS TABLE exige DROP + CREATE. Base = def de mig 277, intacta
-- salvo esos 2 cambios (sigue usando precio_normalizado_shadow = TC nuevo). AISLADA.
--
...
COMMENT ON FUNCTION public.buscar_unidades_simple_shadow(jsonb) IS
  'Clon shadow de buscar_unidades_simple (mig 269 + 277). Sobre propiedades_v2_shadow '
  '+ precio_normalizado_shadow() + v_mercado_venta_shadow. mig 277: + equipado, '
  'uso_inmueble, equipamiento_otros (append al final del RETURNS TABLE).';

GRANT EXECUTE ON FUNCTION public.buscar_unidades_simple_shadow(jsonb) TO service_role, claude_readonly;

-- =============================================================================
-- VERIFICACIÓN (tras aplicar):
--   SELECT id, nombre_proyecto, pet_friendly, amenities_lista
--   FROM public.buscar_unidades_simple_shadow('{}'::jsonb)
--   WHERE pet_friendly LIMIT 5;   -- pet_friendly=true y 'Pet Friendly' NO debe estar en amenities_lista
-- =============================================================================
-- ROLLBACK (vuelve a la firma de mig 277, sin pet_friendly):
--   DROP FUNCTION IF EXISTS public.buscar_unidades_simple_shadow(jsonb);
--   -- y re-aplicar mig 277.
-- =============================================================================
