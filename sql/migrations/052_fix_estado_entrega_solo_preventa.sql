-- =====================================================
-- MIGRACION 052: Fix filtro estado_entrega + solo_preventa
-- Fecha: 13 Enero 2026
-- BASADA EN: migracion 048 (COMPLETA)
-- Cambio: 3 opciones claras para estado_entrega
--   - entrega_inmediata: Solo listas (nuevo, usado, entrega inmediata)
--   - solo_preventa: Solo preventa (inversores)
--   - no_importa: Todo el mercado
-- =====================================================
...
BEGIN
  ...
  WHERE p.es_activa = true
    ...
    -- =====================================================
    -- FIX 052: Filtro estado_entrega con 3 opciones MOAT
    -- entrega_inmediata: Solo listas (excluye preventa)
    -- solo_preventa: Solo preventa (para inversores)
    -- no_importa (o null): Todo el mercado
    -- =====================================================
    AND (
      CASE
        ...
        THEN p.estado_construccion::text = 'preventa'
...
'v2.18: Fix estado_entrega con 3 opciones MOAT.
- entrega_inmediata: Solo listas para mudarse ya
- solo_preventa: Solo preventa (inversores, precios bajos)
- no_importa: Todo el mercado
Basada en v2.17 (048). Excluye duplicados y Sin zona.';

-- =====================================================
-- TEST
-- =====================================================

SELECT 'Test estado_entrega MOAT:' as info;

SELECT 'entrega_inmediata' as filtro, COUNT(*) as cantidad
FROM buscar_unidades_reales('{"estado_entrega": "entrega_inmediata", "limite": 500}');

SELECT 'solo_preventa' as filtro, COUNT(*) as cantidad
FROM buscar_unidades_reales('{"estado_entrega": "solo_preventa", "limite": 500}');

SELECT 'no_importa' as filtro, COUNT(*) as cantidad
FROM buscar_unidades_reales('{"estado_entrega": "no_importa", "limite": 500}');

SELECT 'Migracion 052 completada' as status;
