-- ============================================================
-- DRY-RUN — Migration 229 (filtros area_min / area_max alquiler)
-- Instrucciones:
--   1. Seleccionar TODO este archivo (Ctrl+A), copiar (Ctrl+C).
--   2. Pegar en Supabase SQL Editor.
--   3. Ejecutar (Run).
--   4. Mirar los counts al final. Si se ven bien:
--      - Baseline esperado ≈ 125
--      - area_45_50 esperado > 0 (props con 45-50 m²)
--   5. Si todo OK → correr el archivo 229_buscar_unidades_alquiler_area_filtro.sql
--      (el original, sin BEGIN/ROLLBACK) para COMMITEAR de verdad.
-- ============================================================
...
AS $function$
  ...
  BEGIN
      ...
      WHERE p.es_activa = true
        ...
        -- Migration 229: area_min / area_max (deep-link filters)
        AND (
        ...
        -- Migration 213: filtro por nombre de edificio/proyecto
        AND (
...
-- Validación (no se guarda por el ROLLBACK):
SELECT 'baseline'    AS label, count(*) AS n FROM buscar_unidades_alquiler('{"limite":9999}'::jsonb)
UNION ALL
SELECT 'area_45_50'  AS label, count(*) AS n FROM buscar_unidades_alquiler('{"area_min":45,"area_max":50,"limite":9999}'::jsonb)
UNION ALL
SELECT 'area_60plus' AS label, count(*) AS n FROM buscar_unidades_alquiler('{"area_min":60,"limite":9999}'::jsonb);
