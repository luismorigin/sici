-- =====================================================
-- MIGRACION 062: Filtro de días en mercado por confianza
-- Fecha: 19 Enero 2026
-- Documentación: docs/canonical/FILTROS_CONFIANZA_DATOS.md
-- =====================================================
-- Problema: Anuncios "zombies" que no se bajan después de venderse
-- Ejemplo: Las Dalias con 753 días en mercado (entrega_inmediata)
--
-- Solución: Cortes máximos según estado de construcción
--   - Preventa: 24 meses (730 días) - proyectos pueden retrasarse
--   - Otros: 10 meses (300 días) - si no se vendió, hay problema
--
-- Parámetro: incluir_datos_viejos (default false)
-- =====================================================
-- EJECUTAR EN SUPABASE SQL EDITOR
-- =====================================================

-- Verificar antes: propiedades que serán excluidas
SELECT 'ANTES: Propiedades que serán excluidas por días en mercado:' as info;
SELECT
  pm.nombre_oficial as proyecto,
  p.estado_construccion,
  COUNT(*) as unidades,
  MAX(CURRENT_DATE - COALESCE(p.fecha_publicacion, p.fecha_discovery::date)) as dias_max
...
WHERE p.es_activa = true
  ...
  AND (
    (p.estado_construccion = 'preventa' AND CURRENT_DATE - COALESCE(p.fecha_publicacion, p.fecha_discovery::date) > 730)
    OR
    (COALESCE(p.estado_construccion, 'no_especificado') != 'preventa' AND CURRENT_DATE - COALESCE(p.fecha_publicacion, p.fecha_discovery::date) > 300)
  )
GROUP BY pm.nombre_oficial, p.estado_construccion
...
BEGIN
  ...
  WHERE p.es_activa = true
    ...
    -- =====================================================
    -- v2.8: FILTRO DIAS EN MERCADO (CONFIANZA)
    -- Preventa: 24 meses (730 días)
    -- Otros: 10 meses (300 días)
    -- Documentación: docs/canonical/FILTROS_CONFIANZA_DATOS.md
    -- =====================================================
    AND (
...
'v2.8: Filtro de confianza por días en mercado.
- Preventa: máx 24 meses (730 días)
- Otros estados: máx 10 meses (300 días)
- Parámetro: incluir_datos_viejos para desactivar
- Documentación: docs/canonical/FILTROS_CONFIANZA_DATOS.md
Filtros: tipo_operacion, dormitorios, precio, area, zona, zonas_permitidas, microzona,
proyecto, desarrollador, solo_con_telefono, solo_con_fotos, score_min, estado_entrega,
incluir_outliers, incluir_multiproyecto, incluir_datos_viejos';

-- =====================================================
-- VERIFICACION
-- =====================================================

SELECT 'DESPUES: Las Dalias debe estar excluida:' as info;
SELECT id, proyecto, dias_en_mercado, estado_construccion
FROM buscar_unidades_reales('{"proyecto": "Dalias", "limite": 10}'::jsonb);

SELECT 'DESPUES: MARE debe estar incluida (preventa < 730 días):' as info;
SELECT id, proyecto, dias_en_mercado, estado_construccion
FROM buscar_unidades_reales('{"proyecto": "MARE", "limite": 10}'::jsonb);

SELECT 'DESPUES: Total propiedades confiables:' as info;
SELECT COUNT(*) as total_confiables
FROM buscar_unidades_reales('{"limite": 500}'::jsonb);

SELECT 'DESPUES: Con incluir_datos_viejos=true (debe incluir Las Dalias):' as info;
SELECT id, proyecto, dias_en_mercado, estado_construccion
FROM buscar_unidades_reales('{"proyecto": "Dalias", "incluir_datos_viejos": true, "limite": 10}'::jsonb);

SELECT 'Migracion 062 completada - Filtro dias en mercado activo' as status;
