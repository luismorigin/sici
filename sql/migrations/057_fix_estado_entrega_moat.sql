-- =====================================================
-- MIGRACION 057: Fix estado_entrega - lógica MOAT
-- Fecha: 13 Enero 2026
--
-- BUG: "entrega_inmediata" solo mostraba 126 propiedades
--      (las que explícitamente dicen entrega_inmediata/nuevo/usado)
--
-- FIX MOAT: Mostrar TODO lo que NO sea preventa (240 props)
--           Porque no_especificado y NULL podrían estar listas
-- =====================================================
-- EJECUTAR EN SUPABASE SQL EDITOR
-- =====================================================

-- Solo actualizar la parte del WHERE que filtra estado_entrega
-- Reemplaza la función completa con el fix
...
BEGIN
  ...
  proyecto_stats AS (
    ...
    JOIN proyectos_master pmx ON pv.id_proyecto_master = pmx.id_proyecto_master
    WHERE pv.status = 'completado'
      AND pv.es_activa = true
      AND pv.area_total_m2 > 20
      AND pmx.zona != 'Sin zona'
  ...
  SELECT
    ...
    (SELECT COUNT(*) + 1
     ...
     JOIN proyectos_master pmx ON otras.id_proyecto_master = pmx.id_proyecto_master
  ...
  WHERE p.es_activa = true
    ...
    -- FIX MOAT: estado_entrega con 3 opciones claras
    AND (
      CASE
        WHEN p_filtros->>'estado_entrega' IS NULL
          OR p_filtros->>'estado_entrega' = 'no_importa'
        THEN true  -- TODO el mercado (~264)
        WHEN p_filtros->>'estado_entrega' = 'solo_preventa'
        THEN p.estado_construccion::text = 'preventa'  -- SOLO preventa (~102)
        WHEN p_filtros->>'estado_entrega' = 'entrega_inmediata'
        THEN (
          -- MOAT: TODO lo que NO sea preventa (~240)
          -- Incluye: entrega_inmediata, nuevo_a_estrenar, usado, no_especificado, NULL
          p.estado_construccion IS NULL
          OR p.estado_construccion::text != 'preventa'
        )
        ELSE true
      END
    )
...
'v2.17: Fix estado_entrega MOAT - entrega_inmediata ahora excluye preventa
en lugar de incluir solo las etiquetadas. Incluye no_especificado y NULL.';

-- Test: Verificar las 3 opciones
SELECT 'Test estado_entrega:' as test;

SELECT 'no_importa (todo)' as filtro, COUNT(*) as cantidad
FROM buscar_unidades_reales('{"estado_entrega": "no_importa", "limite": 500}'::jsonb)
UNION ALL
SELECT 'entrega_inmediata (no preventa)', COUNT(*)
FROM buscar_unidades_reales('{"estado_entrega": "entrega_inmediata", "limite": 500}'::jsonb)
UNION ALL
SELECT 'solo_preventa', COUNT(*)
FROM buscar_unidades_reales('{"estado_entrega": "solo_preventa", "limite": 500}'::jsonb);
