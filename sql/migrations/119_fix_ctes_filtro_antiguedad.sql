-- ============================================================================
-- Migración 119: Fix CTEs en buscar_unidades_reales() - Filtrar propiedades antiguas
-- ============================================================================
-- Problema: Los 4 CTEs (proyecto_stats, zona_stats, edificio_stats, tipologia_stats)
--           no filtran propiedades antiguas, causando que métricas como "posición en
--           edificio" y "unidades más baratas" incluyan datos obsoletos de 768+ días
-- Solución: Agregar filtro de días (300 para normales, 730 para preventa) a cada CTE
-- ============================================================================

-- Función auxiliar para verificar si una propiedad es antigua
-- (útil para reutilizar la lógica sin repetir el CASE)
...
BEGIN
    RETURN CASE
        WHEN p_estado_construccion = 'preventa' THEN
            CURRENT_DATE - COALESCE(p_fecha_publicacion, p_fecha_discovery::date) <= 730
        ELSE
            CURRENT_DATE - COALESCE(p_fecha_publicacion, p_fecha_discovery::date) <= 300
    END;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- IMPORTANTE: Eliminar función existente porque la firma puede diferir
DROP FUNCTION IF EXISTS buscar_unidades_reales(jsonb);

-- Redefinir buscar_unidades_reales con filtro de antigüedad en CTEs
CREATE OR REPLACE FUNCTION buscar_unidades_reales(p_filtros jsonb DEFAULT '{}'::jsonb)
RETURNS TABLE(
    id INTEGER,
...
BEGIN
  ...
  WITH
  -- ========================================================================
  -- CTE 1: proyecto_stats - AHORA CON FILTRO DE ANTIGÜEDAD
  -- ========================================================================
  proyecto_stats AS (
    SELECT
  ...
  -- ========================================================================
  -- CTE 2: zona_stats - AHORA CON FILTRO DE ANTIGÜEDAD
  -- ========================================================================
  zona_stats AS (
    SELECT
      pmz.zona,
  ...
  -- ========================================================================
  -- CTE 3: edificio_stats - AHORA CON FILTRO DE ANTIGÜEDAD
  -- ========================================================================
  edificio_stats AS (
    SELECT
      pv.id_proyecto_master,
  ...
  -- ========================================================================
  -- CTE 4: tipologia_stats - AHORA CON FILTRO DE ANTIGÜEDAD
  -- ========================================================================
  tipologia_stats AS (
    SELECT
      pv.id_proyecto_master,
  ...
  SELECT
    ...
    -- SUBQUERY: posicion_precio_edificio - AHORA CON FILTRO DE ANTIGÜEDAD
    (SELECT COUNT(*) + 1
    ...
    WHERE otras.id_proyecto_master = p.id_proyecto_master
      ...
      AND COALESCE(otras.precio_usd_actualizado, otras.precio_usd) < COALESCE(p.precio_usd_actualizado, p.precio_usd)
      -- NUEVO: Filtro de antigüedad
    ...
    -- SUBQUERY: posicion_en_tipologia - AHORA CON FILTRO DE ANTIGÜEDAD
    (SELECT COUNT(*) + 1
    FROM propiedades_v2 otras
    WHERE otras.id_proyecto_master = p.id_proyecto_master
    ...
    -- MIGRACIÓN 115: Leer de amenities_lista directamente
    CASE
      WHEN jsonb_typeof(p.datos_json->'amenities'->'lista') = 'array'
    ...
    -- FIX MIGRACIÓN 116: Leer equipamiento de la lista guardada
    CASE
...
DO $$
BEGIN
    RAISE NOTICE 'Migración 110 completada: buscar_unidades_reales() ahora filtra propiedades antiguas en CTEs y subqueries';
END $$;
