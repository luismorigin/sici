-- =====================================================
-- MIGRACION 033: Fix filtro solo_con_fotos
-- Fecha: 11 Enero 2026
-- Problema: jsonb_array_length() falla si fotos_urls es JSON null
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    -- Fotos como array de TEXT (con check de tipo)
    CASE
      ...
      ELSE ARRAY[]::TEXT[]
    END,
    -- Cantidad de fotos (con check de tipo)
    CASE
      WHEN jsonb_typeof(p.datos_json->'contenido'->'fotos_urls') = 'array'
  ...
  WHERE p.es_activa = true
    ...
    -- =====================================================
    -- FIX: Filtro solo_con_fotos con CASE (PostgreSQL no cortocircuita AND)
    -- =====================================================
    AND (
...
'v2.3: Fix solo_con_fotos para manejar JSON null.
...
SELECT 'Test: solo_con_fotos=true' as test;
SELECT COUNT(*) as con_fotos FROM buscar_unidades_reales('{"solo_con_fotos": true, "limite": 500}');

SELECT 'Test: solo_con_fotos=false' as test;
SELECT COUNT(*) as sin_filtro FROM buscar_unidades_reales('{"solo_con_fotos": false, "limite": 500}');

SELECT 'Migracion 033 completada' as status;
