-- =====================================================
-- MIGRACION 063: buscar_unidades_reales COMPLETA
-- Fecha: 19 Enero 2026
-- Proposito: Combinar TODOS los campos MOAT + filtros de confianza
-- BASADA EN: 048 (campos completos) + 062 (filtro días confianza)
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    -- AMENITIES CONFIRMADOS (fix jsonb_each crash)
    CASE
    ...
    -- AMENITIES POR VERIFICAR (fix jsonb_each crash)
    CASE
      WHEN jsonb_typeof(p.datos_json->'amenities'->'estado_amenities') = 'object'
      THEN (SELECT ARRAY_AGG(key ORDER BY key)
            FROM jsonb_each(p.datos_json->'amenities'->'estado_amenities') AS x(key, val)
    ...
    -- EQUIPAMIENTO DETECTADO (de descripción)
  ...
  WHERE p.es_activa = true
    ...
    -- =====================================================
    -- FILTRO: Excluir "Sin zona" (fuera del polígono actual)
    -- =====================================================
    AND pm.zona != 'Sin zona'

    -- =====================================================
    -- FILTRO: Excluir duplicados
    -- =====================================================
    ...
    -- =====================================================
    -- FILTRO CONFIANZA: Días en mercado (de migración 062)
    -- Preventa: 24 meses (730 días) - construcción tarda
    -- Otros: 10 meses (300 días) - si sigue, algo está mal
...
'v2.20 COMPLETA: Todos los campos MOAT + filtros de confianza.
- Campos: descripcion, posicion_mercado, amenities_confirmados/por_verificar, equipamiento_detectado
- Comparación: edificio (unidades, posicion, min/max) y tipología
- Filtros: Sin zona excluido, duplicados excluidos, días confianza (300/730)
- Parámetros: incluir_datos_viejos, incluir_outliers, incluir_multiproyecto
Documentación: docs/canonical/FILTROS_CONFIANZA_DATOS.md';

-- =====================================================
-- TEST: Verificar que funciona
-- =====================================================
SELECT 'Migracion 063 - Test completo:' as status;

SELECT COUNT(*) as total_props,
       COUNT(*) FILTER (WHERE descripcion IS NOT NULL) as con_descripcion,
       COUNT(*) FILTER (WHERE posicion_mercado IS NOT NULL) as con_posicion_mercado,
       COUNT(*) FILTER (WHERE amenities_confirmados IS NOT NULL) as con_amenities,
       COUNT(*) FILTER (WHERE equipamiento_detectado IS NOT NULL) as con_equipamiento
FROM buscar_unidades_reales('{"limite": 200}'::jsonb);

SELECT 'Verificar exclusion Sin zona:' as check_zona;
SELECT COUNT(*) as props_sin_zona
