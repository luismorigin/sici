-- ============================================================================
-- Migración 197: Merge consume LLM tipo_cambio_detectado
--
-- Agrega tipo_cambio_detectado del LLM a la cadena de prioridad del merge.
-- Incluye: candado, protección contra degradación, upgrade-only con confianza alta.
--
-- Cambios (sobre v2.4.0):
--   1. tipo_cambio_detectado: candado → regex específico → LLM (alta) → existing protected → fallback
--   2. Candado check para tipo_cambio_detectado (antes no existía)
--   3. Protección degradación: nunca sobreescribir paralelo/oficial con no_especificado
--   4. Tracking: fuente_tipo_cambio en cambios_merge y datos_json
--
-- Versión merge: 2.4.0 → 2.5.0
-- ============================================================================
...
BEGIN
    ...
    -- -----------------------------------------------------------------
    -- PRECIO USD (Regla especial con fallback) — sin cambios
    -- -----------------------------------------------------------------
    ...
    ELSIF v_llm_dormitorios IS NOT NULL
      ...
      AND v_llm_dormitorios IS DISTINCT FROM v_disc_dormitorios THEN
        -- LLM con confianza alta corrige discovery (ej: monoambientes 1→0)
    ...
    ELSIF v_llm_dormitorios IS NOT NULL THEN
        -- LLM como fallback si no hay discovery
        v_dormitorios_final := v_llm_dormitorios;
        v_fuente_dormitorios := 'llm';
        v_campos_updated := array_append(v_campos_updated, 'dormitorios');
    ...
    -- -----------------------------------------------------------------
    -- NOMBRE EDIFICIO (v2.4.0: Discovery > Enrichment > LLM > existing)
    -- -----------------------------------------------------------------
    ...
    ELSIF v_llm_nombre_edificio IS NOT NULL AND v_llm_nombre_edificio != '' THEN
        -- LLM como fallback cuando discovery y regex no tienen nombre
        v_nombre_edificio_final := v_llm_nombre_edificio;
    ...
    -- -----------------------------------------------------------------
    -- ESTADO CONSTRUCCIÓN (v2.4.0: LLM con protección)
    -- Regla: NUNCA degradar entrega_inmediata → preventa desde LLM
    -- -----------------------------------------------------------------
    ...
    IF NOT _is_campo_bloqueado(v_candados, 'estado_construccion')
       ...
       AND v_llm_estado_construccion != '' THEN
        -- Protección: nunca degradar entrega_inmediata → preventa
        IF v_prop.estado_construccion::TEXT = 'entrega_inmediata'
           AND v_llm_estado_construccion = 'preventa' THEN
            -- Mantener entrega_inmediata, LLM probablemente lee anuncio desactualizado
            v_estado_construccion_final := v_prop.estado_construccion;
            v_fuente_estado_construccion := 'existing_protected';
            v_campos_kept := array_append(v_campos_kept, 'estado_construccion');
        ELSE
            -- Aplicar LLM: cubre no_especificado→X, nuevo_a_estrenar→entrega_inmediata, preventa→entrega_inmediata
    ...
    -- -----------------------------------------------------------------
    -- TIPO CAMBIO DETECTADO (v2.5.0: candado + LLM upgrade)
    -- Regla: LLM solo UPGRADES (no_especificado → paralelo/oficial).
    --         Regex specific always wins (audit: 0 contradictions).
    --         NEVER downgrade a specific value to no_especificado.
    -- -----------------------------------------------------------------
    ...
    UPDATE propiedades_v2 SET
        ...
        -- Tipo de cambio (v2.5.0: resuelto con LLM + protección)
        tipo_cambio_detectado = v_tipo_cambio_final,
        tipo_cambio_usado = v_enr_tipo_cambio_usado,
        depende_de_tc = CASE
        ...
        -- v2.4.0: LLM directo para solo_tc_paralelo y es_multiproyecto
...
-- Mantener el overload integer → text
...
-- ============================================================================
-- Correr en Supabase SQL Editor después de deployar:
--
-- 1. Dry test con prop completada (no va a ejecutar porque status='completado'):
--    SELECT merge_discovery_enrichment('458');
--
-- 2. Para test real, temporalmente cambiar status de una prop:
--    UPDATE propiedades_v2 SET status = 'actualizado' WHERE id = 458;
--    SELECT merge_discovery_enrichment('458');
--    -- Verificar cambios_merge.fuentes para ver si usó 'llm'
--    SELECT datos_json->'trazabilidad'->>'merge_version',
--           cambios_merge->'fuentes'
--    FROM propiedades_v2 WHERE id = 458;
