-- =====================================================
-- MIGRACION 059: Excluir duplicados en buscar_unidades_reales
-- Fecha: 18 Enero 2026
-- Problema: SANTORINI VENTURA devuelve 18 registros cuando solo hay 2 originales
-- Solucion: Agregar filtro duplicado_de IS NULL
-- =====================================================
-- EJECUTAR EN SUPABASE SQL EDITOR
-- =====================================================

-- Verificar antes: cuantos duplicados hay actualmente
SELECT 'ANTES: Propiedades con duplicado_de NOT NULL que pasaban:' as info;
SELECT COUNT(*) as duplicados_incluidos
...
WHERE p.es_activa = true
  ...
  AND p.duplicado_de IS NOT NULL;
...
RETURNS TABLE (
  ...
  -- v2.6: Campo para debug de duplicados
...
BEGIN
  ...
  -- IMPORTANTE: Solo contar originales para el promedio
  proyecto_stats AS (
    SELECT
    ...
    WHERE pv.status = 'completado'
      ...
      AND pv.duplicado_de IS NULL  -- Solo originales para el promedio
...
'v2.6: Excluye duplicados (duplicado_de IS NULL).
- Corrige bug donde SANTORINI VENTURA mostraba 18 registros en vez de 2
- Stats de proyecto/zona ahora solo usan originales
- Nuevo campo: es_duplicado (para debug)
...
SELECT 'DESPUES: SANTORINI VENTURA (deberia ser 2, no 18):' as info;
SELECT id, proyecto, precio_m2, dormitorios, es_duplicado
FROM buscar_unidades_reales('{"proyecto": "SANTORINI", "limite": 50}'::jsonb);

SELECT 'DESPUES: Total propiedades (sin duplicados):' as info;
SELECT COUNT(*) as total_sin_duplicados
FROM buscar_unidades_reales('{"limite": 500}'::jsonb);

SELECT 'Migracion 059 completada - Duplicados excluidos' as status;
