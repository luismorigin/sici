-- Migración 246: Guardrail monoambiente en merge_discovery_enrichment (VENTA)
-- Problema: props que el portal publica como "monoambiente" quedan con dormitorios=1
--           (deberían ser 0). El backfill ya corrigió las existentes; esto es el fix
--           estructural/preventivo. La función de venta NO tenía guardrail (a diferencia
--           de merge_alquiler, que lo tiene desde la migración 214).
-- Fix: post-resolución de v_dormitorios_final, si el título (URL) o el crudo de discovery
--      dice "monoambiente" y quedó >=1 dormitorio, forzar a 0. Respeta candados.
-- Riesgo: bajo — solo agrega un IF aditivo, no modifica ninguna otra lógica del merge.
--         Precedente directo: migración 214 hizo esto mismo en alquiler.
-- Rollback: quitar el bloque "GUARDRAIL monoambiente" y recrear la función.
--
-- BASE: versión de PRODUCCIÓN exportada con pg_get_functiondef('merge_discovery_enrichment(text)'::regprocedure)
--       (version_merge 2.6.0). El guardrail se insertó tras el bloque DORMITORIOS
--       (que termina con v_fuente_dormitorios := 'existing') y antes del bloque BAÑOS.
--       NOTA: existe un overload merge_discovery_enrichment(integer) que solo delega a
--       esta versión de text — NO requiere cambios.
...
BEGIN
    ...
    -- GUARDRAIL monoambiente: si el título (URL) o el crudo dice "monoambiente"
    -- y quedó >=1 dormitorio, forzar a 0. Respeta candados.
    IF NOT _is_campo_bloqueado(v_candados, 'dormitorios')
       AND v_dormitorios_final >= 1
       AND (v_prop.url ILIKE '%monoambiente%'
            OR v_prop.datos_json_discovery::text ILIKE '%monoambiente%')
    THEN
        v_dormitorios_final := 0;
        v_fuente_dormitorios := 'monoambiente_guardrail';
    END IF;
