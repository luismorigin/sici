-- ============================================================================
-- MIGRACIÓN 109: Equipamiento - Leer datos guardados del admin
-- ============================================================================
-- Fecha: 3 Febrero 2026
-- Propósito: Que buscar_unidades_reales() lea equipamiento guardado en admin
-- Problema: Actualmente solo detecta desde descripción con ILIKE, ignorando
--           los datos que el usuario guarda en datos_json->'amenities'->'equipamiento'
-- Solución: Priorizar datos guardados, fallback a detección si no hay datos
-- Base: Migración 105 (buscar_unidades_reales v2.30)
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    -- =====================================================
    -- v2.31: EQUIPAMIENTO - Priorizar datos guardados
    -- Si existe datos_json->'amenities'->'equipamiento', usarlo
    -- Fallback: detectar desde descripción (solo ~6% de propiedades)
    -- =====================================================
    CASE
      -- PRIMERO: Usar equipamiento guardado por admin si existe y tiene elementos
      WHEN jsonb_typeof(p.datos_json->'amenities'->'equipamiento') = 'array'
           AND jsonb_array_length(p.datos_json->'amenities'->'equipamiento') > 0
      THEN ARRAY(
        SELECT jsonb_array_elements_text(p.datos_json->'amenities'->'equipamiento')
        ORDER BY 1
      )
      -- FALLBACK: Detectar desde descripción (para propiedades sin datos guardados)
      ELSE (SELECT ARRAY_AGG(item ORDER BY item)
            FROM (
  ...
  WHERE p.es_activa = true
    ...
    -- v2.30: FILTRO UNIFICADO 300 DÍAS PARA TODOS
    AND (
      (p_filtros->>'incluir_datos_viejos')::boolean IS TRUE
      OR CURRENT_DATE - COALESCE(p.fecha_publicacion, p.fecha_discovery::date) <= 300
    )
    AND (
...
COMMENT ON FUNCTION buscar_unidades_reales IS 'v2.31: Equipamiento lee datos guardados del admin, fallback a detección desde descripción';
