-- ============================================================================
-- MIGRACIÓN 101: buscar_unidades_reales() v2.27 - Plan de Pagos Cuotas
-- ============================================================================
-- Fecha: 2026-02-01
-- Propósito: Agregar campos plan_pagos_cuotas y plan_pagos_texto al retorno
-- Dependencia: Migración 084 (función v2.26)
-- ============================================================================

-- =====================================================
-- 1. NUEVAS COLUMNAS EN propiedades_v2
-- =====================================================

ALTER TABLE propiedades_v2
ADD COLUMN IF NOT EXISTS plan_pagos_cuotas JSONB DEFAULT NULL;

COMMENT ON COLUMN propiedades_v2.plan_pagos_cuotas IS
'Array de cuotas: [{id, porcentaje, momento, descripcion}]. NULL si plan_pagos_desarrollador=false o sin detalle';

ALTER TABLE propiedades_v2
ADD COLUMN IF NOT EXISTS plan_pagos_texto TEXT DEFAULT NULL;

COMMENT ON COLUMN propiedades_v2.plan_pagos_texto IS
'Texto legible del plan de pagos, generado desde cuotas o manual';

-- =====================================================
-- 2. ACTUALIZAR buscar_unidades_reales() v2.27
-- =====================================================
...
BEGIN
  ...
  WHERE table_name = 'propiedades_v2'
    AND column_name IN ('plan_pagos_cuotas', 'plan_pagos_texto');
  ...
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ MIGRACIÓN 101: buscar_unidades_reales v2.27 - Plan de Pagos';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '📋 Columnas agregadas: % de 2', v_cols;
  RAISE NOTICE '📋 Función actualizada: %', v_func;
  RAISE NOTICE '';
  RAISE NOTICE '📊 Nuevos campos en retorno:';
  RAISE NOTICE '   - plan_pagos_cuotas (JSONB)';
  RAISE NOTICE '   - plan_pagos_texto (TEXT)';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
END $$;
