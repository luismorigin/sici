-- ============================================================================
-- Migración 149: Orden default por fecha_publicacion en buscar_unidades_alquiler()
-- ============================================================================
-- Cambio: ORDER BY 'recientes' usa fecha_publicacion en vez de fecha_discovery
-- Motivo: fecha_discovery refleja cuándo el pipeline encontró la propiedad,
--         fecha_publicacion refleja cuándo se publicó el anuncio (más relevante
--         para el usuario)
-- Fallback: COALESCE con fecha_discovery::date por si alguna propiedad no tiene
--           fecha_publicacion (actualmente 100% cobertura)
-- ============================================================================
...
BEGIN
    ...
    IF p_filtros->'zonas_permitidas' IS NOT NULL
       AND jsonb_array_length(p_filtros->'zonas_permitidas') > 0 THEN
        ...
        -- Check if sin_zona was requested (to also match NULLs)
    ...
    WHERE p.es_activa = true
      ...
      -- Filtro: dormitorios (NULL dormitorios siempre pasan)
      AND (
      ...
      -- Filtro: zonas (incluir NULLs cuando sin_zona seleccionado)
      AND (
    ...
    ORDER BY
        ...
        -- CAMBIO: fecha_publicacion en vez de fecha_discovery
...
DO $$
BEGIN
    RAISE NOTICE 'Migración 149: Orden default fecha_publicacion en buscar_unidades_alquiler()';
END $$;
