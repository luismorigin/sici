-- ============================================================================
-- MIGRACIÓN 079: buscar_unidades_reales() usa precio_usd_actualizado
-- ============================================================================
-- Fecha: 2026-01-28
-- Propósito: Usar COALESCE(precio_usd_actualizado, precio_usd) para que
--            los precios se actualicen dinámicamente con el TC de Binance
-- Riesgo: BAJO - COALESCE hace fallback a precio_usd si no hay actualizado
-- ============================================================================
-- Cambios quirúrgicos:
--   1. CTEs de stats: usar precio con COALESCE
--   2. SELECT principal: precio_usd con COALESCE
--   3. precio_m2 calculado: usar COALESCE
--   4. Subqueries posición edificio/tipología: usar COALESCE
--   5. es_precio_outlier: usar COALESCE
--   6. WHERE filtros precio: usar COALESCE
--   7. ORDER BY: usar COALESCE
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    p.banos,
    -- ========================================================================
    -- CAMBIO PRINCIPAL: usar precio actualizado si existe
    -- ========================================================================
    ...
    -- Sin cambios - usa descripción, no precio
    -- =====================================================
...
BEGIN
  ...
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ MIGRACIÓN 079: buscar_unidades_reales con precio_usd_actualizado';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '📋 Función actualizada: %', v_func_exists;
  RAISE NOTICE '📋 Cambios: COALESCE(precio_usd_actualizado, precio_usd) en:';
  RAISE NOTICE '   - CTEs de estadísticas (proyecto, zona, edificio, tipología)';
  RAISE NOTICE '   - SELECT principal (precio_usd, precio_m2)';
  RAISE NOTICE '   - Subqueries de posición';
  RAISE NOTICE '   - Cálculo es_precio_outlier';
  RAISE NOTICE '   - Filtros WHERE precio_min/precio_max';
  RAISE NOTICE '   - ORDER BY';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '⚠️  NOTA: Si precio_usd_actualizado es NULL, usa precio_usd';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
END $$;
...
-- Ver propiedades con precio actualizado vs original
SELECT
  id,
  precio_usd as original,
  precio_usd_actualizado as actualizado,
  COALESCE(precio_usd_actualizado, precio_usd) as usado
FROM propiedades_v2
