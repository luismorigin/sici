-- ============================================================================
-- Migración 153: Soporte multi-selección dormitorios en buscar_unidades_alquiler()
-- ============================================================================
-- Feature: Permitir seleccionar múltiples opciones de dormitorios (ej: "1 + 3+")
-- Nuevo parámetro: dormitorios_lista (jsonb array, ej: [0,1] o [1,3])
-- Si el array contiene 3, se interpreta como "3+" (>= 3)
-- Compatible: dormitorios y dormitorios_min siguen funcionando como fallback
-- ============================================================================
...
BEGIN
    ...
    IF p_filtros->'dormitorios_lista' IS NOT NULL
       ...
       AND jsonb_array_length(p_filtros->'dormitorios_lista') > 0 THEN
        ...
        -- Separate exact matches (0,1,2) from 3+ flag
    ...
    SELECT
        ...
        -- Amenities: fallback propiedad → proyecto
        CASE
    ...
    WHERE p.es_activa = true
      ...
      -- Filtro: dormitorios (multi-select con dormitorios_lista, fallback a dormitorios/dormitorios_min)
      AND (
          CASE
              -- Nuevo: dormitorios_lista (multi-select)
...
DO $$
BEGIN
    RAISE NOTICE 'Migración 153: Multi-selección dormitorios en buscar_unidades_alquiler() (dormitorios_lista jsonb array)';
END $$;
