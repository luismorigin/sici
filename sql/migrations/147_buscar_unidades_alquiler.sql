-- ============================================================================
-- Migración 147: buscar_unidades_alquiler()
-- ============================================================================
-- RPC para el feed de alquileres. Versión simplificada de buscar_unidades_reales()
-- sin CTEs de estadísticas, sin posición mercado, sin razón fiduciaria.
-- Incluye mapeo de microzonas (nombres viejos/nuevos en BD).
-- ============================================================================
...
BEGIN
    ...
    SELECT
        ...
        -- Fotos: mismo patrón que buscar_unidades_reales
        CASE
    ...
    ORDER BY
        ...
        CASE WHEN p_filtros->>'orden' IS NULL OR p_filtros->>'orden' = 'recientes' THEN p.fecha_discovery END DESC NULLS LAST,
        p.id DESC
    LIMIT COALESCE((p_filtros->>'limite')::int, 50);
END;
$function$;

-- Permisos para anon (landing pública)
GRANT EXECUTE ON FUNCTION buscar_unidades_alquiler(jsonb) TO anon;
GRANT EXECUTE ON FUNCTION buscar_unidades_alquiler(jsonb) TO authenticated;

DO $$
BEGIN
    RAISE NOTICE 'Migración 147: Creada buscar_unidades_alquiler() con mapeo microzonas';
END $$;
