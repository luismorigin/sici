-- =============================================================================
-- Migración 276 (SHADOW) — RPC feed alquiler: exponer 5 campos para el frontend
-- -----------------------------------------------------------------------------
-- La mig 275 devolvía amoblado/acepta_mascotas/amenities_lista/equipamiento_lista
-- pero NO: equipado, uso_inmueble, expensas_incluidas, ni las colas
-- (amenities_extra, equipamiento_otros) → el front no podía mostrarlos
-- (ej. "expensas incluidas" no aparecía: 96 props lo tenían en datos_json).
--
-- Este mig re-crea buscar_unidades_alquiler_shadow AGREGANDO 5 columnas al final
-- del RETURNS TABLE (append → no rompe el orden posicional de las existentes):
--   equipado boolean · uso_inmueble text · expensas_incluidas boolean
--   amenities_extra text[] · equipamiento_otros text[]
-- Todos salen de datos_json (ya canonicalizado in-place por lib/canonicalizar.mjs).
-- Cambiar el RETURNS TABLE exige DROP + CREATE (no se puede CREATE OR REPLACE).
-- Base = def de mig 275, intacta salvo el append. AISLADA de venta.
--
...
--   SELECT id, amoblado, expensas_incluidas, equipado, uso_inmueble, amenities_extra, equipamiento_otros
--   FROM public.buscar_unidades_alquiler_shadow('{}'::jsonb) LIMIT 5;
-- =============================================================================
-- ROLLBACK (vuelve a la firma de mig 275, sin los 5 campos):
--   DROP FUNCTION IF EXISTS public.buscar_unidades_alquiler_shadow(jsonb);
--   -- y re-aplicar mig 275.
-- =============================================================================
