-- =============================================================================
-- Migración 281 (SHADOW) — RPC feed VENTA: días desde captura ("Nuevo" vs "Reciente")
-- -----------------------------------------------------------------------------
-- Sigue a mig 280. Re-crea buscar_unidades_simple_shadow con UN cambio:
--   + columna `dias_desde_captura integer` (= CURRENT_DATE - fecha_creacion), justo
--     después de `dias_en_mercado` en el RETURNS TABLE y en el SELECT.
--
-- Motivo: separar dos conceptos que hoy están colapsados en el badge de la card:
--   · "Nuevo"    = recién capturado por NOSOTROS  → fecha_creacion (estable, NO se
--                  pisa como fecha_discovery, que se reescribe con NOW() cada noche).
--   · "Reciente" = recién publicado en el portal   → dias_en_mercado (fecha_publicacion).
-- El front (ventas.tsx) prende "Nuevo" con dias_desde_captura ≤ 3, y baja el umbral
-- de "Reciente" a 30d SOLO cuando esta columna viaja (shadow). En prod la RPC no la
-- devuelve → badge histórico intacto (60d, sin "Nuevo").
--
-- Cambiar el RETURNS TABLE exige DROP + CREATE. Base = def de mig 280, intacta salvo
-- esa columna (sigue usando precio_normalizado_shadow = TC nuevo). AISLADA de prod.
--
...
CREATE FUNCTION public.buscar_unidades_simple_shadow(p_filtros jsonb DEFAULT '{}'::jsonb)
 RETURNS TABLE(id integer, nombre_proyecto text, desarrollador text, zona text, microzona text, dormitorios integer, banos numeric, precio_usd numeric, precio_m2 numeric, area_m2 numeric, score_calidad integer, agente_nombre text, agente_telefono text, agente_oficina text, fotos_urls text[], fotos_count integer, url text, amenities_lista jsonb, es_multiproyecto boolean, estado_construccion text, dias_en_mercado integer, dias_desde_captura integer, amenities_confirmados text[], amenities_por_verificar text[], equipamiento_detectado text[], descripcion text, latitud numeric, longitud numeric, estacionamientos integer, baulera boolean, fecha_entrega text, piso text, plan_pagos_desarrollador boolean, acepta_permuta boolean, solo_tc_paralelo boolean, precio_negociable boolean, descuento_contado_pct numeric, parqueo_incluido boolean, parqueo_precio_adicional numeric, baulera_incluido boolean, baulera_precio_adicional numeric, plan_pagos_cuotas jsonb, plan_pagos_texto text, fuente text, tc_sospechoso boolean, equipado boolean, uso_inmueble text, equipamiento_otros text[], pet_friendly boolean)
 LANGUAGE plpgsql
AS $function$
  BEGIN
    RETURN QUERY
    ...
    SELECT
      ...
      -- Amenities lista (v280: sin "Pet Friendly" → es chip pet_friendly, no amenidad)
      CASE
        WHEN jsonb_typeof(p.datos_json->'amenities'->'lista') = 'array'
        THEN COALESCE((SELECT jsonb_agg(e) FROM jsonb_array_elements(p.datos_json->'amenities'->'lista') e WHERE e <> '"Pet Friendly"'::jsonb), '[]'::jsonb)
        ELSE '[]'::jsonb
      END,
      ...
      -- v281: días desde que NOSOTROS lo capturamos (fecha_creacion, estable).
      -- Alimenta el badge "Nuevo" en el feed. NO usar fecha_discovery (se pisa cada noche).
      (CURRENT_DATE - p.fecha_creacion::date)::INTEGER as dias_desde_captura,
      ...
      CASE
        ...
        THEN (SELECT ARRAY_AGG(elem ORDER BY elem)
              ...
              WHERE elem <> 'Pet Friendly')   -- v280: Pet Friendly = chip propio
        ELSE NULL
      END::TEXT[],
      ...
      -- Equipamiento detectado (canónico)
      CASE
      ...
      -- ── v277: campos nuevos para el frontend (de datos_json ya poblado) ──
      (p.datos_json->>'equipado')::boolean,                          -- flag electrodomésticos (true/null)
      p.datos_json->>'uso_inmueble',                                 -- residencial | mixto (null en deptos hoy; útil al escalar)
      CASE
      ...
      COALESCE(pm.pet_friendly, false)                               -- v280: chip pet-friendly del EDIFICIO (derivado, mig 278)
...
-- GRANTs (mismo patrón que las RPC shadow previas)
GRANT EXECUTE ON FUNCTION public.buscar_unidades_simple_shadow(jsonb) TO anon, authenticated, service_role;

-- =============================================================================
-- ROLLBACK — volver a la def de mig 280 (sin dias_desde_captura):
--   Re-aplicar sql/migrations/280_shadow_venta_rpc_pet_friendly.sql
--   (el front tolera la ausencia de la columna: dias_desde_captura = null → sin badge "Nuevo").
-- =============================================================================
