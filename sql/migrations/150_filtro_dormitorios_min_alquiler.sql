-- ============================================================================
-- Migración 150: Filtro dormitorios_min en buscar_unidades_alquiler()
-- ============================================================================
-- Bug: botón "3+" no filtraba porque frontend enviaba dormitorios=3 pero
--      la condición era `d < 3` → nunca se enviaba.
-- Fix: nuevo parámetro dormitorios_min para filtrar >= N dormitorios.
--      dormitorios sigue siendo match exacto (0, 1, 2).
-- Incluye también el cambio de migración 149 (ORDER BY fecha_publicacion).
-- ============================================================================
...
BEGIN
    ...
    WHERE p.es_activa = true
      ...
      -- Filtro: dormitorios exacto (0, 1, 2) — NULL dormitorios siempre pasan
      AND (
          p_filtros->>'dormitorios' IS NULL
          OR p.dormitorios = (p_filtros->>'dormitorios')::int
          OR p.dormitorios IS NULL
      )
      -- Filtro: dormitorios mínimo (3+) — NULL dormitorios NO pasan
      AND (
          p_filtros->>'dormitorios_min' IS NULL
          OR p.dormitorios >= (p_filtros->>'dormitorios_min')::int
      )
      -- Filtro: amoblado
      AND (
...
DO $$
BEGIN
    RAISE NOTICE 'Migración 150: Filtro dormitorios_min (3+) en buscar_unidades_alquiler()';
END $$;
