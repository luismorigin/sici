-- =====================================================
-- MIGRACION 054: buscar_unidades_reales - pasar precio/m2 a posicion_mercado
-- Fecha: 13 Enero 2026
-- Proposito: Actualizar llamada a calcular_posicion_mercado para pasar precio/m2
--
-- DEPENDE DE: migracion 053 (fix calcular_posicion_mercado)
--
-- CAMBIO: Linea 252 de migracion 047
-- ANTES:  calcular_posicion_mercado(p.precio_usd, pm.zona, p.dormitorios)
-- AHORA:  calcular_posicion_mercado(p.precio_usd / NULLIF(p.area_total_m2, 0), pm.zona, p.dormitorios)
-- =====================================================
-- EJECUTAR EN SUPABASE SQL EDITOR (DESPUES DE 053)
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    -- v2.14: CORREGIDO - Pasar precio/m2 en lugar de precio total
    calcular_posicion_mercado(
      p.precio_usd / NULLIF(p.area_total_m2, 0),  -- CAMBIO: precio/m2
      pm.zona,
      p.dormitorios
    ) as posicion_mercado
...
'v2.14: CORREGIDO - posicion_mercado ahora recibe precio/m2 (no precio total).
Esto hace que el % vs mercado sea comparable entre departamentos de distinto tamano.';

-- =====================================================
-- TESTS
-- =====================================================

-- Test 1: Verificar Sky Moon (id conocido con $3,029/m2)
SELECT 'Test 1: Verificar posicion_mercado para mono en Equipetrol Norte/Norte' as test;
SELECT
  id,
  proyecto,
  zona,
  ...
  posicion_mercado->>'posicion_texto' as posicion_texto,
  posicion_mercado->'contexto'->>'precio_m2_consultado' as precio_m2_consultado,
  posicion_mercado->'contexto'->>'precio_m2_promedio_zona' as precio_m2_promedio_zona
FROM buscar_unidades_reales('{"dormitorios": 0, "zona": "Equipetrol Norte", "limite": 5}'::jsonb);

-- Test 2: Verificar que el calculo es coherente
SELECT 'Test 2: Monos en Equipetrol - verificar % coherente' as test;
SELECT
  proyecto,
  precio_m2,
  posicion_mercado->>'diferencia_pct' as dif_pct,
  posicion_mercado->>'categoria' as categoria
FROM buscar_unidades_reales('{"dormitorios": 0, "zona": "Equipetrol", "limite": 10}'::jsonb)
ORDER BY precio_m2;

SELECT 'Migracion 054 completada - buscar_unidades_reales v2.14' as status;
SELECT 'posicion_mercado ahora compara precio/m2 (coherente con tamano)' as resultado;
