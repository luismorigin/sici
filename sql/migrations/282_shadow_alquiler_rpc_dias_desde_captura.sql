-- =============================================================================
-- Migración 282 (SHADOW) — RPC feed ALQUILER: días desde captura ("Nuevo" vs "Reciente")
-- -----------------------------------------------------------------------------
-- Gemela de mig 281 (ventas). Re-crea buscar_unidades_alquiler_shadow con UN cambio:
...
--
-- Motivo: unificar los badges de la card y del bottom sheet, que hoy están
-- colapsados/inconsistentes (card dice "Nuevo" ≤60d publicación, sheet "Reciente"
-- ≤60d → misma prop, dos nombres). Separar:
--   · "Nuevo"    = recién capturado por NOSOTROS  → fecha_creacion (estable).
--   · "Reciente" = recién publicado en el portal   → dias_en_mercado.
-- El front (alquileres.tsx) prende "Nuevo" con dias_desde_captura ≤ 3 y baja el
-- umbral de "Reciente" a 30d SOLO cuando esta columna viaja (shadow). En prod la
-- RPC no la devuelve → badge histórico intacto.
--
-- Cambiar el RETURNS TABLE exige DROP + CREATE. Base = def vigente, intacta salvo
-- esa columna. AISLADA de prod.
--
...
CREATE FUNCTION public.buscar_unidades_alquiler_shadow(p_filtros jsonb DEFAULT '{}'::jsonb)
 RETURNS TABLE(id integer, nombre_edificio text, nombre_proyecto text, desarrollador text, zona text, dormitorios integer, banos numeric, area_m2 numeric, precio_mensual_bob numeric, precio_mensual_usd numeric, amoblado text, acepta_mascotas boolean, deposito_meses numeric, servicios_incluidos jsonb, contrato_minimo_meses integer, monto_expensas_bob numeric, piso integer, estacionamientos integer, baulera boolean, latitud numeric, longitud numeric, fotos_urls text[], fotos_count integer, url text, fuente text, agente_nombre text, agente_telefono text, agente_whatsapp text, dias_en_mercado integer, dias_desde_captura integer, estado_construccion text, id_proyecto_master integer, amenities_lista text[], equipamiento_lista text[], descripcion text, equipado boolean, uso_inmueble text, expensas_incluidas boolean, amenities_extra text[], equipamiento_otros text[], pet_friendly boolean)
 LANGUAGE plpgsql
 STABLE
AS $function$
  DECLARE
  ...
  BEGIN
      ...
      SELECT
          ...
          -- v282: días desde que NOSOTROS lo capturamos (fecha_creacion, estable).
          -- Alimenta el badge "Nuevo". NO usar fecha_discovery (se pisa cada noche).
          (CURRENT_DATE - p.fecha_creacion::date)::INTEGER,
...
GRANT EXECUTE ON FUNCTION public.buscar_unidades_alquiler_shadow(jsonb) TO anon, authenticated, service_role;

-- =============================================================================
-- ROLLBACK — volver a la def previa (sin dias_desde_captura):
--   Re-aplicar la migración shadow de alquiler anterior (275_shadow_alquiler_buscar_unidades.sql
--   + los parches posteriores 276/279 que tocaron esta RPC).
