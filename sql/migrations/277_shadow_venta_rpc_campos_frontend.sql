-- =============================================================================
-- Migración 277 (SHADOW) — RPC feed VENTA: exponer 3 campos para el frontend
-- -----------------------------------------------------------------------------
-- Gemela de la mig 276 (que hizo lo mismo en alquiler). La RPC de venta
-- (buscar_unidades_simple_shadow, mig 269) NO exponía: equipado (flag), la cola
-- equipamiento_otros, ni uso_inmueble. Este mig los agrega AL FINAL del
-- RETURNS TABLE (append → no rompe el orden posicional existente):
--   equipado boolean · uso_inmueble text · equipamiento_otros text[]
-- Todos salen de datos_json (ya poblado: equipado 397/397, equipamiento_otros
-- 397/397; uso_inmueble hoy null en deptos, útil al escalar a casas/mixto).
-- expensas_incluidas NO se agrega en venta (no tiene sentido: una venta no
-- incluye expensas mensuales → siempre null).
--
-- Cambiar el RETURNS TABLE exige DROP + CREATE. Base = def de mig 269, intacta
-- salvo el append (sigue llamando precio_normalizado_shadow de mig 272 por
-- nombre → toma el TC nuevo). AISLADA de alquiler.
--
...
--   SELECT id, nombre_proyecto, equipado, uso_inmueble, equipamiento_otros
--   FROM public.buscar_unidades_simple_shadow('{}'::jsonb) LIMIT 5;
-- =============================================================================
-- ROLLBACK (vuelve a la firma de mig 269, sin los 3 campos):
--   DROP FUNCTION IF EXISTS public.buscar_unidades_simple_shadow(jsonb);
--   -- y re-aplicar mig 269 (la def de buscar_unidades_simple_shadow).
-- =============================================================================
