-- Migración 214: Guardrail monoambiente en merge_alquiler
-- Problema: Haiku 4.5 clasifica monoambientes como 1D en ~15 casos (ignora regla del prompt)
-- Fix: post-LLM override en merge — si descripcion_limpia o URL dice "monoambiente" y dorms=1, forzar a 0
-- Riesgo: bajo — solo agrega un IF, no modifica lógica existente
-- Rollback: quitar el bloque IF y recrear la función
...
BEGIN
    ...
    LOOP
        ...
        -- Si el LLM clasificó como 1D pero la descripción o URL dice "monoambiente", corregir a 0.
        -- Respeta candados (si está bloqueado, no se toca).
        -- ===========================
