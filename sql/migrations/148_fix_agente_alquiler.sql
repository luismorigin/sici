-- =====================================================
-- Migración 148: Poblar datos_json->agente en alquileres
-- Propósito: El merge_alquiler() no construía el bloque 'agente'
--   en datos_json, dejando nombre/telefono/inmobiliaria vacíos
--   en el admin editor. Los datos SÍ existen en datos_json_discovery.
-- Fecha: 18 Feb 2026
-- Prerequisito: 138
-- SEGURIDAD:
--   PARTE 1: Solo agrega key 'agente' en datos_json, NO toca otro campo.
--            Respeta datos manuales existentes (no sobreescribe si ya hay nombre).
--   PARTE 2: CREATE OR REPLACE merge_alquiler() — copia EXACTA de 138
--            con 4 líneas agregadas para extraer agente desde discovery.
-- =====================================================

-- =====================================================
-- PARTE 1: UPDATE retroactivo — poblar agente desde datos_json_discovery
-- =====================================================

-- 1A: Century21 (asesorNombre, telefono, nombreAfiliado)
UPDATE propiedades_v2
SET datos_json = COALESCE(datos_json, '{}'::jsonb) || jsonb_build_object(
    'agente', jsonb_build_object(
        'nombre', datos_json_discovery->>'asesorNombre',
        'telefono', datos_json_discovery->>'telefono',
        'oficina_nombre', datos_json_discovery->>'nombreAfiliado'
    )
)
WHERE tipo_operacion = 'alquiler'
  AND fuente = 'century21'
  AND datos_json_discovery->>'asesorNombre' IS NOT NULL
  AND datos_json_discovery->>'asesorNombre' != ''
  -- No sobreescribir si ya tiene agente con nombre real
  AND (
    datos_json->'agente'->>'nombre' IS NULL
    OR datos_json->'agente'->>'nombre' = ''
  );

-- 1B: Remax (agent->user->name_to_show, agent->office->name)
UPDATE propiedades_v2
SET datos_json = COALESCE(datos_json, '{}'::jsonb) || jsonb_build_object(
    'agente', jsonb_build_object(
        'nombre', datos_json_discovery->'agent'->'user'->>'name_to_show',
        'telefono', NULL::TEXT,
        'oficina_nombre', datos_json_discovery->'agent'->'office'->>'name'
    )
)
WHERE tipo_operacion = 'alquiler'
  AND fuente = 'remax'
  AND datos_json_discovery->'agent'->'user'->>'name_to_show' IS NOT NULL
  AND datos_json_discovery->'agent'->'user'->>'name_to_show' != ''
  -- No sobreescribir si ya tiene agente con nombre real
  AND (
    datos_json->'agente'->>'nombre' IS NULL
    OR datos_json->'agente'->>'nombre' = ''
...
-- =====================================================
-- PARTE 2: Actualizar merge_alquiler() para futuros merges
-- Cambios vs migración 138 (marcados con -- [148]):
--   1. SELECT agrega pv.fuente
--   2. Bloque "AGENTE" nuevo después de NOMBRE EDIFICIO
--   3. datos_json agrega bloque agente
--   4. version '1.2.0'
-- =====================================================
...
DECLARE
    ...
    v_agente_telefono TEXT;
    v_agente_oficina TEXT;
...
BEGIN
    ...
    FOR v_rec IN
        SELECT
            pv.id,
            pv.status,
            pv.fuente,  -- [148] necesario para extraer agente por fuente
    ...
    LOOP
        ...
        SET
            ...
            datos_json = COALESCE(datos_json, '{}'::jsonb)
                ...
                || CASE WHEN (v_enrichment->'llm_output'->'servicios_incluidos') IS NOT NULL
                    ...
                    ELSE '{}'::jsonb END
                -- [148] AGENTE desde discovery
                || jsonb_build_object(
                    'agente', jsonb_build_object(
                        'nombre', v_agente_nombre,
                        'telefono', v_agente_telefono,
                        'oficina_nombre', v_agente_oficina
                    )
                )
                || jsonb_build_object(
                    'merge_alquiler', jsonb_build_object(
                        'fuentes', v_fuentes,
                        'timestamp', NOW(),
                        'version', '1.2.0'  -- [148] bump version
                    )
                ),
        ...
        RETURN QUERY SELECT
            ...
            'merged'::TEXT,
            format(
                'Merge alquiler OK: precio=%s BOB, area=%s m², dorms=%s, agente=%s',
                COALESCE(v_precio_bob::TEXT, 'NULL'),
                COALESCE(v_area::TEXT, 'NULL'),
                COALESCE(v_dorms::TEXT, 'NULL'),
                COALESCE(v_agente_nombre, 'NULL')
            )::TEXT;
...
COMMENT ON FUNCTION merge_alquiler IS 'Merge alquiler v1.2.0: agrega bloque agente desde discovery. Enrichment-first, sin TC paralelo, sin score fiduciario.';

-- =====================================================
-- VERIFICACIÓN
-- =====================================================
/*
-- Verificar retroactivo: cuántos tienen agente ahora
SELECT
  COUNT(*) as total,
  COUNT(*) FILTER (WHERE datos_json->'agente'->>'nombre' IS NOT NULL AND datos_json->'agente'->>'nombre' != '') as con_nombre
FROM propiedades_v2
WHERE tipo_operacion = 'alquiler' AND status = 'completado';

-- Ver algunos ejemplos
SELECT id, fuente, datos_json->'agente' as agente
FROM propiedades_v2
WHERE tipo_operacion = 'alquiler' AND status = 'completado'
LIMIT 10;
*/
