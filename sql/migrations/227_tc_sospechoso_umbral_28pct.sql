-- Migración 227: ajustar umbral tc_sospechoso de 30% a 28%
-- Propiedades al borde del umbral (ej. 29% debajo de mediana) quedaban
-- fuera del badge "Confirmar TC" cuando en la práctica eran claramente
-- sospechosas. Bajar a 28% captura esas sin aumentar ruido significativo.
--
-- Único cambio vs migración 219: línea con `* 0.70` → `* 0.72`
-- (threshold = precio/m2 < mediana × 0.72 → debajo del 28%).
...
GRANT EXECUTE ON FUNCTION public.buscar_unidades_simple(jsonb) TO anon, authenticated, service_role;
