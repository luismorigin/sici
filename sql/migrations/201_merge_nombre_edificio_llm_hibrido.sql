-- =====================================================================
-- Migración 201: merge nombre_edificio — LLM híbrido
-- Fecha: 2026-03-27
-- Versión merge: 2.5.0 → 2.6.0
--
-- PROBLEMA: nombre_edificio usa prioridad Discovery > Enrichment > LLM,
-- pero discovery/regex frecuentemente extrae basura ("Venta", "Calle X",
-- "El CorazóN De", etc). LLM con confianza alta tiene el nombre correcto
-- en ~91% de los casos, pero solo se usa como fallback cuando no hay nada.
--
-- SOLUCIÓN (híbrida): LLM con confianza alta gana sobre discovery/regex
-- cuando:
--   a) El nombre discovery/regex es "sospechoso" (patrones de basura), O
--   b) El nombre discovery/regex no matchea ningún proyecto_master existente
--
-- Si el nombre discovery/regex SÍ matchea un PM, se respeta (safe path).
--
-- IMPACTO: ~10 de 12 props sin match se benefician inmediatamente.
-- Props ya matcheadas con nombre correcto en discovery NO se afectan.
-- =====================================================================

-- =====================================================================
-- 1. Helper: detectar nombres sospechosos de discovery/regex
-- =====================================================================
CREATE OR REPLACE FUNCTION _is_nombre_edificio_sospechoso(p_nombre TEXT)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT CASE
        WHEN p_nombre IS NULL OR TRIM(p_nombre) = '' THEN true
        -- Tipo de operación en vez de nombre
        WHEN LOWER(TRIM(p_nombre)) ~ '^(venta|pre[ -]?venta|preventa|alquiler|arriendo)' THEN true
        -- Dirección en vez de nombre
        WHEN LOWER(TRIM(p_nombre)) ~ '^(calle |av[. ]|avenida |barrio |zona )' THEN true
        -- Marketing fluff conocido
        WHEN LOWER(TRIM(p_nombre)) ~ '^(eco sostenible|el coraz)' THEN true
        -- CamelCase roto típico de extractores: "AgustíN CastañAres"
        WHEN p_nombre ~ '[a-záéíóúñü][A-ZÁÉÍÓÚÑÜ][a-záéíóúñü].*[A-ZÁÉÍÓÚÑÜ]' THEN true
        ELSE false
    END;
$$;

-- =====================================================================
-- 2. Helper: verificar si un nombre existe en proyectos_master
--    (por nombre_oficial o alias_conocidos, usando normalize_nombre)
-- =====================================================================
CREATE OR REPLACE FUNCTION _nombre_existe_en_proyectos(p_nombre TEXT)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
AS $$
    SELECT EXISTS (
        ...
        WHERE normalize_nombre(pm.nombre_oficial) = normalize_nombre(p_nombre)
    )
    OR EXISTS (
        SELECT 1 FROM proyectos_master pm,
               unnest(COALESCE(pm.alias_conocidos, ARRAY[]::text[])) alias
        WHERE normalize_nombre(alias) = normalize_nombre(p_nombre)
    );
$$;

-- =====================================================================
-- 3. Merge v2.6.0 — nombre_edificio LLM híbrido
-- =====================================================================
CREATE OR REPLACE FUNCTION public.merge_discovery_enrichment(p_identificador text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
DECLARE
    ...
    -- LLM output (Fase C)
    v_llm JSONB;
    v_llm_dormitorios INTEGER;
    v_llm_dormitorios_confianza TEXT;
    v_llm_estado_construccion TEXT;
    v_llm_nombre_edificio TEXT;
    v_llm_nombre_edificio_confianza TEXT;  -- NEW v2.6.0
    v_llm_solo_tc_paralelo BOOLEAN;
    v_llm_es_multiproyecto BOOLEAN;
    v_llm_tipo_cambio_detectado TEXT;
    v_llm_tipo_cambio_confianza TEXT;
    ...
    v_zona_final TEXT;
    v_estado_construccion_final estado_construccion_enum;
    v_tipo_cambio_final TEXT;

    -- v2.6.0: nombre edificio híbrido
    v_mejor_nombre_no_llm TEXT;
    ...
    v_fuente_estado_construccion TEXT := 'none';
    v_fuente_tipo_cambio TEXT := 'none';
...
BEGIN
    ...
    -- =========================================================================
    -- FASE 2: EXTRAER VALORES DE ENRICHMENT (regex)
    -- =========================================================================
    ...
    -- =========================================================================
    -- FASE 2.5: EXTRAER VALORES DE LLM OUTPUT (Fase C - v2.4.0)
    -- =========================================================================

    v_llm := v_enrichment->'llm_output';

    IF v_llm IS NOT NULL AND v_llm != 'null'::JSONB THEN
        v_llm_dormitorios := CASE
            WHEN v_llm->>'dormitorios' ~ '^[0-9]+$'
            THEN (v_llm->>'dormitorios')::INTEGER
            ELSE NULL
        END;
        v_llm_dormitorios_confianza := v_llm->>'dormitorios_confianza';
        v_llm_estado_construccion := v_llm->>'estado_construccion';
        v_llm_nombre_edificio := v_llm->>'nombre_edificio';
        v_llm_nombre_edificio_confianza := v_llm->>'nombre_edificio_confianza';  -- NEW v2.6.0
        v_llm_solo_tc_paralelo := (v_llm->>'solo_tc_paralelo')::BOOLEAN;
        v_llm_es_multiproyecto := (v_llm->>'es_multiproyecto')::BOOLEAN;
        v_llm_tipo_cambio_detectado := v_llm->>'tipo_cambio_detectado';
        v_llm_tipo_cambio_confianza := v_llm->>'tipo_cambio_confianza';
    END IF;
    ...
    -- -----------------------------------------------------------------
    -- ÁREA (Discovery > Enrichment) — sin cambios
    -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
    -- DORMITORIOS (v2.4.0: candado → LLM alta → discovery → regex)
    -- -----------------------------------------------------------------
    ...
    ELSIF v_llm_dormitorios IS NOT NULL
      AND v_llm_dormitorios_confianza = 'alta'
      AND v_llm_dormitorios IS DISTINCT FROM v_disc_dormitorios THEN
        v_dormitorios_final := v_llm_dormitorios;
        v_fuente_dormitorios := 'llm';
    ...
    ELSIF v_llm_dormitorios IS NOT NULL THEN
        v_dormitorios_final := v_llm_dormitorios;
        v_fuente_dormitorios := 'llm';
        v_campos_updated := array_append(v_campos_updated, 'dormitorios');
    ...
    -- -----------------------------------------------------------------
    -- BAÑOS (Discovery > Enrichment) — sin cambios
    -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
    -- ESTACIONAMIENTOS (Discovery > Enrichment) — sin cambios
    -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
    -- GPS - LATITUD (Discovery > Enrichment) — sin cambios
    -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
    -- GPS - LONGITUD (Discovery > Enrichment) — sin cambios
    -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
    -- NOMBRE EDIFICIO (v2.6.0: candado → LLM híbrido → disc → enr → LLM fallback)
    --
    -- Cambio vs v2.5.0: LLM con confianza alta gana sobre discovery/regex
    -- cuando el nombre discovery/regex es sospechoso (basura) o no matchea
    -- ningún proyecto_master existente. Si el nombre discovery/regex SÍ
    -- matchea un PM, se respeta (safe path).
    -- -----------------------------------------------------------------
    ...
    ELSE
        -- Determinar el mejor nombre NO-LLM disponible
        v_mejor_nombre_no_llm := COALESCE(
            NULLIF(TRIM(v_disc_nombre_edificio), ''),
            NULLIF(TRIM(v_enr_nombre_edificio), '')
        );

        IF v_llm_nombre_edificio IS NOT NULL AND TRIM(v_llm_nombre_edificio) != ''
           AND v_llm_nombre_edificio_confianza = 'alta'
           AND v_mejor_nombre_no_llm IS NOT NULL
           AND (
               -- Path A: nombre discovery/regex es basura detectada
               _is_nombre_edificio_sospechoso(v_mejor_nombre_no_llm)
               OR
               -- Path B: nombre discovery/regex no matchea ningún PM
               NOT _nombre_existe_en_proyectos(v_mejor_nombre_no_llm)
           ) THEN
            -- LLM híbrido: gana porque discovery/regex no es confiable
            v_nombre_edificio_final := TRIM(v_llm_nombre_edificio);
            v_fuente_nombre_edificio := 'llm';
            v_campos_updated := array_append(v_campos_updated, 'nombre_edificio');

        ELSIF v_mejor_nombre_no_llm IS NOT NULL THEN
            -- Discovery/regex tiene nombre válido que matchea PM → respetar
            v_nombre_edificio_final := v_mejor_nombre_no_llm;
            v_fuente_nombre_edificio := CASE
                WHEN NULLIF(TRIM(v_disc_nombre_edificio), '') IS NOT NULL THEN 'discovery'
                ELSE 'enrichment'
            END;
            v_campos_kept := array_append(v_campos_kept, 'nombre_edificio');

        ELSIF v_llm_nombre_edificio IS NOT NULL AND TRIM(v_llm_nombre_edificio) != '' THEN
            -- LLM como fallback (discovery y regex vacíos)
            v_nombre_edificio_final := TRIM(v_llm_nombre_edificio);
            v_fuente_nombre_edificio := 'llm';
            v_campos_updated := array_append(v_campos_updated, 'nombre_edificio');
    ...
    -- -----------------------------------------------------------------
    -- ZONA (Discovery > Enrichment) — sin cambios
    -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
    -- ESTADO CONSTRUCCIÓN (v2.4.0: LLM con protección) — sin cambios
    -- -----------------------------------------------------------------
    v_estado_construccion_final := v_prop.estado_construccion;
    v_fuente_estado_construccion := 'existing';

    IF NOT _is_campo_bloqueado(v_candados, 'estado_construccion')
       AND v_llm_estado_construccion IS NOT NULL
       AND v_llm_estado_construccion != '' THEN
        IF v_prop.estado_construccion::TEXT = 'entrega_inmediata'
           AND v_llm_estado_construccion = 'preventa' THEN
            v_estado_construccion_final := v_prop.estado_construccion;
            v_fuente_estado_construccion := 'existing_protected';
            v_campos_kept := array_append(v_campos_kept, 'estado_construccion');
        ELSE
            v_estado_construccion_final := v_llm_estado_construccion::estado_construccion_enum;
            v_fuente_estado_construccion := 'llm';
    ...
    -- -----------------------------------------------------------------
    -- TIPO CAMBIO DETECTADO (v2.5.0: candado + LLM upgrade) — sin cambios
    -- -----------------------------------------------------------------
    v_tipo_cambio_final := v_enr_tipo_cambio_detectado;
    v_fuente_tipo_cambio := 'enrichment';

    IF _is_campo_bloqueado(v_candados, 'tipo_cambio_detectado') THEN
        v_tipo_cambio_final := v_prop.tipo_cambio_detectado;
        v_fuente_tipo_cambio := 'blocked';
        v_campos_blocked := array_append(v_campos_blocked, 'tipo_cambio_detectado');

    ELSIF v_enr_tipo_cambio_detectado IN ('paralelo', 'oficial') THEN
        v_tipo_cambio_final := v_enr_tipo_cambio_detectado;
        v_fuente_tipo_cambio := 'enrichment';
        v_campos_kept := array_append(v_campos_kept, 'tipo_cambio_detectado');

    ELSIF v_llm_tipo_cambio_detectado IN ('paralelo', 'oficial')
          AND v_llm_tipo_cambio_confianza = 'alta' THEN
        v_tipo_cambio_final := v_llm_tipo_cambio_detectado;
        v_fuente_tipo_cambio := 'llm';
        v_campos_updated := array_append(v_campos_updated, 'tipo_cambio_detectado');

    ELSIF v_prop.tipo_cambio_detectado IN ('paralelo', 'oficial')
          AND COALESCE(v_enr_tipo_cambio_detectado, 'no_especificado') = 'no_especificado' THEN
        v_tipo_cambio_final := v_prop.tipo_cambio_detectado;
        v_fuente_tipo_cambio := 'existing_protected';
        v_campos_kept := array_append(v_campos_kept, 'tipo_cambio_detectado');
    ...
    -- =========================================================================
    -- FASE 4: CALCULAR DISCREPANCIAS — sin cambios
    -- =========================================================================
    ...
    -- =========================================================================
    -- FASE 5: VERIFICAR DATOS CRÍTICOS FALTANTES — sin cambios
    -- =========================================================================
    ...
    -- =========================================================================
    -- FASE 6: SCORING POST-MERGE — sin cambios
    -- =========================================================================
    ...
    BEGIN
        ...
        IF v_estado_construccion_final IS NOT NULL THEN v_score_core := v_score_core + 10; END IF;

        v_score_opcionales := 0;

        IF v_estacionamientos_final IS NOT NULL THEN v_score_opcionales := v_score_opcionales + 5; END IF;
        IF v_nombre_edificio_final IS NOT NULL THEN v_score_opcionales := v_score_opcionales + 5; END IF;
    ...
    -- =========================================================================
    -- FASE 7: CONSTRUIR DISCOVERY METADATA — sin cambios
    -- =========================================================================
    ...
    -- =========================================================================
    -- FASE 8: CONSTRUIR datos_json FINAL
    -- =========================================================================
    ...
    v_datos_json_final := jsonb_build_object(
        ...
        'financiero', jsonb_build_object(
            ...
            'tipo_cambio_detectado', v_tipo_cambio_final,
            'fuente_tipo_cambio', v_fuente_tipo_cambio,
        ...
        'fisico', jsonb_build_object(
            ...
            'es_multiproyecto', COALESCE(v_llm_es_multiproyecto, v_prop.es_multiproyecto, false),
        ...
        'proyecto', jsonb_build_object(
            ...
            'estado_construccion', v_estado_construccion_final::TEXT,
            'fuente_estado_construccion', v_fuente_estado_construccion
        ),
        ...
        'trazabilidad', jsonb_build_object(
            ...
            'fecha_merge', NOW(),
            'llm_version', v_enrichment->'llm_metadata'->>'version',
            'llm_model', v_enrichment->'llm_metadata'->>'model'
        )
    );

    -- =========================================================================
    -- FASE 9: CONSTRUIR CAMBIOS MERGE
    -- =========================================================================
    ...
    v_cambios_merge := jsonb_build_object(
        ...
        'fuentes', jsonb_build_object(
            ...
            'zona', v_fuente_zona,
            'estado_construccion', v_fuente_estado_construccion,
            'tipo_cambio_detectado', v_fuente_tipo_cambio
        ),
        'datos_criticos_faltantes', v_datos_criticos_faltantes,
        'llm_disponible', (v_llm IS NOT NULL AND v_llm != 'null'::JSONB),
        'timestamp', NOW()
    );
    ...
    UPDATE propiedades_v2 SET
        ...
        -- v2.4.0: Estado construcción desde LLM
        estado_construccion = v_estado_construccion_final,

        -- Tipo de cambio (v2.5.0)
        tipo_cambio_detectado = v_tipo_cambio_final,
        tipo_cambio_usado = v_enr_tipo_cambio_usado,
        depende_de_tc = CASE
            WHEN v_tipo_cambio_final IN ('paralelo', 'oficial')
        ...
        -- v2.4.0: LLM directo
        solo_tc_paralelo = COALESCE(v_llm_solo_tc_paralelo, propiedades_v2.solo_tc_paralelo),
        es_multiproyecto = COALESCE(v_llm_es_multiproyecto, propiedades_v2.es_multiproyecto),
        ...
        es_para_matching = CASE
            WHEN propiedades_v2.id_proyecto_master IS NOT NULL THEN FALSE
    ...
    v_result := jsonb_build_object(
        ...
        'llm_usado', (v_llm IS NOT NULL AND v_llm != 'null'::JSONB),
        'timestamp', NOW()
    );
...
-- Overload INT → TEXT (sin cambios)
CREATE OR REPLACE FUNCTION public.merge_discovery_enrichment(p_id integer)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
-- =====================================================================
-- 4. Guardar función canónica
-- =====================================================================
-- Archivo: sql/functions/merge/merge_discovery_enrichment_v2.6.0.sql
