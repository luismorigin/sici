-- Migration 229: Add area_min / area_max filters to buscar_unidades_alquiler
-- Same pattern as precio_mensual_min/max: (p_filtros->>'key' IS NULL OR ...).
-- Backwards compatible: existing callers that don't pass area_min/max are unaffected.
-- Rollback: sql/migrations/229_buscar_unidades_alquiler_area_filtro_ROLLBACK.sql
...
-- Dry-run validation (optional, run inside BEGIN/ROLLBACK before commit):
-- SELECT count(*) FROM buscar_unidades_alquiler('{}'::jsonb);                    -- baseline
-- SELECT count(*) FROM buscar_unidades_alquiler('{"area_min": 60}'::jsonb);      -- new filter
-- SELECT count(*) FROM buscar_unidades_alquiler('{"area_min": 45, "area_max": 50}'::jsonb);
