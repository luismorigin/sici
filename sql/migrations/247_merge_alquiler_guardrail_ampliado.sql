-- Migración 247: Ampliar señal del guardrail monoambiente en merge_alquiler (ALQUILER)
-- Problema: el guardrail de la migración 214 solo mira llm_output.descripcion_limpia y la URL.
--           Remax/BI publican "monoambiente" en el crudo (nomb_cata / first_address dentro de
--           datos_json_discovery), no siempre en descripcion_limpia, así que esos casos se
--           escapan y quedan con dormitorios=1.
-- Fix: agregar una señal más a la condición OR del guardrail existente:
--           v_rec.datos_json_discovery::text ILIKE '%monoambiente%'
--      (datos_json_discovery ya estaba en el SELECT del loop — no requiere cambios al SELECT).
-- Riesgo: bajo — solo amplía la condición de un IF aditivo ya existente; no toca otra lógica.
--         Hermana de la migración 246 (mismo guardrail recién agregado en venta).
-- Rollback: quitar la línea "OR v_rec.datos_json_discovery::text ILIKE '%monoambiente%'"
--           y recrear la función (vuelve a la versión 1.3.0 de la migración 214).
--
-- BASE: versión de PRODUCCIÓN exportada con pg_get_functiondef('merge_alquiler'::regproc)
--       (idéntica a la migración 214, version 1.3.0 — sin drift). Cambio: señal ampliada
--       en el bloque GUARDRAIL + bump de version a 1.4.0 en metadata merge_alquiler.

CREATE OR REPLACE FUNCTION public.merge_alquiler(p_id integer DEFAULT NULL::integer)
...
BEGIN
    ...
    LOOP
        ...
        -- ===========================
        -- GUARDRAIL: monoambiente = 0 dormitorios
        -- Si el LLM clasificó como 1D pero la descripción, la URL o el CRUDO de discovery
        -- dice "monoambiente", corregir a 0. Respeta candados (si está bloqueado, no se toca).
        -- Migración 247: se amplió la señal con datos_json_discovery::text para cubrir Remax/BI,
        -- que ponen "monoambiente" en nomb_cata / first_address del crudo (no en descripcion_limpia).
        -- ===========================
        IF NOT _is_campo_bloqueado(v_candados, 'dormitorios')
           AND v_dorms = 1
           AND (
             (v_enrichment->'llm_output'->>'descripcion_limpia') ILIKE ANY(ARRAY['%monoambiente%', '%mono ambiente%', '%mono-ambiente%'])
             OR v_rec.col_url ILIKE '%monoambiente%'
             OR v_rec.datos_json_discovery::text ILIKE '%monoambiente%'
           )
        THEN
            v_dorms := 0;
            v_fuentes := v_fuentes || '{"dormitorios_override": "monoambiente_guardrail"}'::jsonb;
        END IF;
