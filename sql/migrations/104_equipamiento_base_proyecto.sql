...
BEGIN
  ...
  -- Hace merge con datos_json->'amenities'->'equipamiento' existente
  -- =========================================================================
  IF p_propagar_equipamiento AND v_proyecto.equipamiento_base IS NOT NULL
    ...
    LOOP
      -- Obtener equipamiento existente o array vacío
      ...
      -- Calcular nuevo equipamiento (unión de existentes + proyecto base)
      SELECT COALESCE(jsonb_agg(DISTINCT value ORDER BY value), '[]'::jsonb)
      INTO v_nuevo_equipamiento
      FROM (
        SELECT jsonb_array_elements_text(v_equipamiento_existente) as value
        UNION
        SELECT jsonb_array_elements_text(v_proyecto.equipamiento_base) as value
      ) combined;
      ...
      INSERT INTO propiedades_v2_historial (
        ...
        v_nuevo_equipamiento,
        'Propagado equipamiento base desde proyecto: ' || v_proyecto_nombre
      );
...
v3.0: Agregado soporte para propagar equipamiento_base.
...
- equipamiento: Se hace merge con datos_json->amenities->equipamiento (NUEVO)';
