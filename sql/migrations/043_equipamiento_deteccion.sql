-- =====================================================
-- MIGRACION 043: Detección de equipamiento en descripciones
-- Fecha: 12 Enero 2026
-- Propósito: Extraer equipamiento mencionado en publicaciones
--            para interpretación fiduciaria
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    -- v2.10: Equipamiento detectado en descripción
    -- NOTA FIDUCIARIA:
    -- - SI detectamos → "La publicación menciona X" (no es exhaustivo)
    -- - SI NO detectamos → NO significa que no tiene (broker puede no haberlo puesto)
    -- =====================================================
...
'v2.10: Detección de equipamiento en descripciones.
- equipamiento_detectado: array de items mencionados en la publicación
- Items detectados: Aire acondicionado, Cocina equipada, Amoblado, Closets empotrados
- NOTA FIDUCIARIA: "detectado" != "incluye todo", solo lo que menciona la publicación';

-- =====================================================
-- TEST: Verificar detección de equipamiento
-- =====================================================
SELECT 'TEST: Verificar detección de equipamiento:' as info;
...
SELECT
  ...
  array_length(equipamiento_detectado, 1) as cant_items
FROM buscar_unidades_reales('{"limite": 15}'::jsonb)
WHERE equipamiento_detectado IS NOT NULL
ORDER BY array_length(equipamiento_detectado, 1) DESC NULLS LAST;

-- Estadísticas de cobertura
SELECT 'Estadísticas de cobertura equipamiento:' as info;

SELECT
  COUNT(*) as total,
  COUNT(*) FILTER (WHERE equipamiento_detectado IS NOT NULL) as con_equipamiento,
  ROUND(100.0 * COUNT(*) FILTER (WHERE equipamiento_detectado IS NOT NULL) / COUNT(*), 1) as pct_cobertura
FROM buscar_unidades_reales('{"limite": 500}'::jsonb);

SELECT 'Migracion 043 completada - equipamiento_detectado disponible' as status;
