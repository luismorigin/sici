-- =====================================================
-- MIGRACIÓN 046: Agregar descripcion a buscar_unidades_reales
-- Fecha: 12 Enero 2026
-- Propósito: Exponer descripción del anunciante para UI
-- BASADA EN: migración 044 (versión completa con todos los campos)
-- =====================================================
...
RETURNS TABLE (
  ...
  descripcion TEXT  -- v2.12: NUEVO - descripción del anunciante
) AS $func$
BEGIN
  RETURN QUERY
  WITH
  ...
  SELECT
    ...
    -- v2.12: NUEVO - Descripción del anunciante (prioridad enrichment > discovery > datos_json)
    COALESCE(
...
'v2.12: Agregar campo descripcion (texto del anunciante) para UI.
Basada en v2.11 con todos los campos: banos, edificio_stats, tipologia_stats, amenities, equipamiento.';

-- Test
SELECT 'Migración 046 - Test descripcion:' as status;
SELECT id, proyecto, LEFT(descripcion, 60) as desc_preview
FROM buscar_unidades_reales('{"dormitorios": 2, "limite": 3}'::jsonb);
