-- =============================================================================
-- Migración 269 — FEED SHADOW (render): vista + RPC que sirven el feed del entorno
-- shadow, para comparar precio_m2 / tc_sospechoso shadow vs real end-to-end.
-- =============================================================================
-- Propósito: cerrar el loop del entorno shadow (mig 268). Clona la vista de mercado
-- y la RPC del feed `/ventas` apuntando a `propiedades_v2_shadow` +
-- `precio_normalizado_shadow()` (TC shadow), sin tocar producción. Así se renderiza
-- el feed shadow y se compara fila-a-fila con el feed real (mismos ids).
--
-- Contiene:
--   1. v_mercado_venta_shadow        — clon de v_mercado_venta sobre la tabla shadow.
--   2. buscar_unidades_simple_shadow — clon EXACTO de buscar_unidades_simple, pero
--      FROM propiedades_v2_shadow, medianas desde v_mercado_venta_shadow y
--      normalización via precio_normalizado_shadow().
--
-- Espeja las definiciones REALES de producción exportadas con pg_get_functiondef/
-- pg_get_viewdef (Regla 7). ÚNICOS cambios: la tabla origen, la función de
-- normalización y la vista de medianas. Filtros/orden/columnas IDÉNTICOS.
--
-- Aislamiento: igual que mig 268 — service_role + claude_readonly, SIN anon/auth
--   → invisible al Data API público. Catálogo (proyectos_master, zonas_geograficas)
--   y helpers (es_propiedad_vigente) son COMPARTIDOS con prod (read-only, OK).
--
...
-- -----------------------------------------------------------------------------
-- 1. VISTA DE MERCADO SHADOW — clon de v_mercado_venta sobre la tabla shadow
-- -----------------------------------------------------------------------------
-- Cambios vs la real: FROM propiedades_v2_shadow + precio_normalizado_shadow().
-- Resto (join zonas_geograficas, filtros de calidad, ventana temporal) IDÉNTICO.
CREATE OR REPLACE VIEW public.v_mercado_venta_shadow AS
  ...
  WHERE (p.status = ANY (ARRAY['completado'::estado_propiedad, 'actualizado'::estado_propiedad])) AND p.tipo_operacion = 'venta'::tipo_operacion_enum AND p.duplicado_de IS NULL AND (lower(COALESCE(p.tipo_propiedad_original, ''::text)) <> ALL (ARRAY['baulera'::text, 'parqueo'::text, 'garaje'::text, 'deposito'::text, 'casa'::text, 'terreno'::text, 'oficina'::text])) AND (p.es_multiproyecto = false OR p.es_multiproyecto IS NULL) AND p.area_total_m2 >= 20::numeric AND p.zona IS NOT NULL AND p.precio_usd > 0::numeric AND
        CASE
            WHEN COALESCE(p.estado_construccion::text, ''::text) = ANY (ARRAY['preventa'::text, 'en_construccion'::text, 'en_pozo'::text]) THEN (CURRENT_DATE - COALESCE(p.fecha_publicacion, p.fecha_discovery::date)) <= 730
            ELSE (CURRENT_DATE - COALESCE(p.fecha_publicacion, p.fecha_discovery::date)) <= 300
        END;

COMMENT ON VIEW public.v_mercado_venta_shadow IS
  'Clon shadow de v_mercado_venta (migración 269): mismo filtro de calidad pero sobre '
  'propiedades_v2_shadow + precio_normalizado_shadow(). Para comparar el feed shadow vs real.';

GRANT SELECT ON public.v_mercado_venta_shadow TO service_role, claude_readonly;

-- -----------------------------------------------------------------------------
-- 2. RPC DEL FEED SHADOW — clon de buscar_unidades_simple sobre la tabla shadow
-- -----------------------------------------------------------------------------
-- Firma de retorno IDÉNTICA a la real (mismo contrato con pages/api/ventas.ts).
-- Cambios: FROM propiedades_v2_shadow, medianas desde v_mercado_venta_shadow,
-- precio_normalizado_shadow() en todos los cálculos. Filtros/orden IDÉNTICOS.
CREATE OR REPLACE FUNCTION public.buscar_unidades_simple_shadow(p_filtros jsonb DEFAULT '{}'::jsonb)
 RETURNS TABLE(id integer, nombre_proyecto text, desarrollador text, zona text, microzona text, dormitorios integer, banos numeric, precio_usd numeric, precio_m2 numeric, area_m2 numeric, score_calidad integer, agente_nombre text, agente_telefono text, agente_oficina text, fotos_urls text[], fotos_count integer, url text, amenities_lista jsonb, es_multiproyecto boolean, estado_construccion text, dias_en_mercado integer, amenities_confirmados text[], amenities_por_verificar text[], equipamiento_detectado text[], descripcion text, latitud numeric, longitud numeric, estacionamientos integer, baulera boolean, fecha_entrega text, piso text, plan_pagos_desarrollador boolean, acepta_permuta boolean, solo_tc_paralelo boolean, precio_negociable boolean, descuento_contado_pct numeric, parqueo_incluido boolean, parqueo_precio_adicional numeric, baulera_incluido boolean, baulera_precio_adicional numeric, plan_pagos_cuotas jsonb, plan_pagos_texto text, fuente text, tc_sospechoso boolean)
 LANGUAGE plpgsql
AS $function$
  BEGIN
    RETURN QUERY
    ...
    SELECT
      ...
      p.dormitorios,
      p.banos,
      precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) as precio_usd,
      ROUND((precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) / NULLIF(p.area_total_m2, 0))::numeric, 0) as precio_m2,
      p.area_total_m2,
      ...
      -- TC sospechoso: no_especificado + precio/m2 >28% debajo de mediana del grupo (migración 227)
      CASE
        WHEN p.tipo_cambio_detectado = 'no_especificado'
             AND m.mediana_m2 IS NOT NULL
             AND (precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) / NULLIF(p.area_total_m2, 0)) < m.mediana_m2 * 0.72
        THEN true
        ELSE false
      END
    ...
    WHERE p.es_activa = true
      ...
      AND (
        ...
        OR precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) <= (p_filtros->>'precio_max')::numeric
      )
      AND (
        p_filtros->>'precio_min' IS NULL
        OR precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) >= (p_filtros->>'precio_min')::numeric
      )
      AND (
    ...
    ORDER BY
      CASE WHEN p_filtros->>'orden' = 'precio_desc' THEN precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) END DESC NULLS LAST,
      CASE WHEN p_filtros->>'orden' = 'recientes' THEN COALESCE(p.fecha_publicacion, p.fecha_discovery::date) END DESC NULLS LAST,
      CASE WHEN p_filtros->>'orden' = 'precio_m2_asc' THEN (precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) / NULLIF(p.area_total_m2, 0)) END ASC NULLS LAST,
      CASE WHEN p_filtros->>'orden' = 'precio_m2_desc' THEN (precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) / NULLIF(p.area_total_m2, 0)) END DESC NULLS LAST,
      CASE WHEN p_filtros->>'orden' IS NULL OR p_filtros->>'orden' = 'precio_asc' THEN precio_normalizado_shadow(p.precio_usd, p.tipo_cambio_detectado) END ASC NULLS LAST,
      p.id DESC
...
COMMENT ON FUNCTION public.buscar_unidades_simple_shadow(jsonb) IS
  'Clon shadow de buscar_unidades_simple (migración 269): mismo contrato de retorno '
  'pero sobre propiedades_v2_shadow + precio_normalizado_shadow() + v_mercado_venta_shadow. '
  'Para renderizar el feed shadow y compararlo con el real end-to-end.';

GRANT EXECUTE ON FUNCTION public.buscar_unidades_simple_shadow(jsonb) TO service_role, claude_readonly;
...
--   DROP VIEW IF EXISTS public.v_mercado_venta_shadow;
-- COMMIT;
-- =============================================================================
